...
class JpaTaskMaterializationAdapterCancellationTest {
    ...
    @Test
    @DisplayName("Given 运行中节点 When 请求并终结取消 Then 先关闭执行权再幂等推进 CANCELED")
    void should_fence_immediately_and_finalize_cancellation_idempotently() {
        ...
        when(executorPlans.findActiveCandidates(TENANT.value(), TASK_ID.value(), "worker"))
                .thenReturn(List.of());
        ...
        var canceling = adapter.requestCancellation(TENANT, USER, TASK_ID, "用户停止", NOW);
        var duplicate = adapter.requestCancellation(TENANT, USER, TASK_ID, "用户停止", NOW);

        assertThat(canceling.status()).isEqualTo(Task.Status.CANCELING);
        assertThat(duplicate.status()).isEqualTo(Task.Status.CANCELING);
        assertThat(canceling.owner()).isEqualTo(new Task.Owner(Task.OwnerKind.HUMAN, USER.value()));
        assertThat(nodeEntity.getStatus()).isEqualTo(TaskPlan.Status.RUNNING.name());
        assertThat(executionEntity.getExecution().status()).isEqualTo(Execution.Status.RUNNING);
        assertThat(dispatchEntity.getDispatch().status()).isEqualTo(TaskDispatch.Status.CANCELED);
        assertThat(dispatchEntity.getDispatch().generation()).isEqualTo(3);
        assertThat(dispatchEntity.getDispatch().fencingToken()).isEqualTo(11);
        assertThat(dispatchEntity.getDispatch().accepts("dispatch-1", 2, 10, "worker-1", NOW))
                .isFalse();

        assertThat(adapter.finalizeCancellation(TENANT, TASK_ID, LATER)).isTrue();
        assertThat(adapter.finalizeCancellation(TENANT, TASK_ID, LATER)).isFalse();

        assertThat(taskEntity.getTask().status()).isEqualTo(Task.Status.CANCELED);
        assertThat(nodeEntity.getStatus()).isEqualTo(TaskPlan.Status.CANCELED.name());
        assertThat(executionEntity.getExecution().status()).isEqualTo(Execution.Status.CANCELED);
        verify(dispatches, times(1)).nextFence();
        verify(eventWriter, times(3)).append(any(ExecutionEvent.class), anyLong());
    }

    private static Task task() {
        var owner = new Task.Owner(Task.OwnerKind.ASSISTANT, "assistant-1");
        return new Task(
                TENANT,
                USER,
                TASK_ID,
