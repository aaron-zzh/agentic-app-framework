 ...
 *
 * <p>认知心理学对齐：人类记忆编码不是简单的"存入"，而是经过注意筛选、 与已有记忆比对、决定新建/更新/遗忘的主动过程。本服务模拟这一过程。
 *
 ...
 *
 * <ol>
 *   <li>注意筛选：MemoryExtractionService 已完成（只提取有价值的事实）
 *   <li>激活已有记忆：向量检索相关已有记忆
 *   <li>编码决策：LLM 比对新旧，决定操作类型
 *   <li>执行：新增/更新/删除/跳过
 * </ol>
 */
@Slf4j
...
public class MemoryDeduplicationService {
    ...
    /**
     * 去重：过滤掉与已有记忆重复的原子，返回需要写入的原子列表。
     *
     * @param atoms 待去重的原子列表
     * @param userId 用户 ID
     * @return 去重后需要写入的原子列表
     */
    public List<MemoryAtom> deduplicate(List<MemoryAtom> atoms, Long userId) {
        var decisions = encodeDecisions(userId, atoms);
        return decisions.stream()
                .filter(d -> d.action() == Action.ADD || d.action() == Action.UPDATE)
                .map(
                        d -> {
                            var atom = new MemoryAtom();
                            atom.setId(d.atomId());
                            atom.setContent(d.content());
    ...
    public List<DecisionResult> encodeDecisions(Long userId, List<MemoryAtom> newAtoms) {
        ...
        for (var atom : newAtoms) {
            ...
            // 激活已有记忆（向量检索相关候选）
            var candidates =
                    atomEngine.searchByVector(userId, atom.getEmbedding(), CANDIDATE_COUNT);
    ...
    public MemoryAtom deduplicateOrPass(MemoryAtom atom) {
        ...
        var candidates =
                atomEngine.searchByVector(atom.getUserId(), atom.getEmbedding(), CANDIDATE_COUNT);
        ...
        try {
            ...
            for (int i = 0; i < candidates.size(); i++) {
                existingText.append(
                        "[%s] %s\n"
                                .formatted(
                                        candidates.get(i).getId().toString(),
            ...
            var messages =
                    List.of(
                            (org.springframework.ai.chat.messages.Message)
                                    new SystemMessage("只返回 JSON，不要其他内容。"),
    ...
    private DecisionResult executeDecision(
            Decision decision, MemoryAtom newAtom, List<MemoryAtom> candidates) {
