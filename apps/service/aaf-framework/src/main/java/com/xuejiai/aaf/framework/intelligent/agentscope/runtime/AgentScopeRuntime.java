 ...
 *
 * <h3>职责</h3>
 * <p>将 AAF 领域对象 {@link AgentDefinition}（DB 元数据）编译为 AgentScope {@link ReActAgent}（可执行实例）。
 * 上层只依赖 {@link AgentRuntime} 接口，本类是该接口在 AgentScope 技术栈上的唯一实现。
 *
 * <h3>构建流程</h3>
 * <pre>
 ...
 * <h3>两种输出方式</h3>
 * <ul>
 *   <li>{@link #create} → 包装为 {@link AgentScopeAgentAdapter}（实现 {@link AgentExecutor}），供 AgentPool 使用
 *   <li>{@link #createRaw} → 返回原始 {@link ReActAgent}，供 AG-UI Registry 注册（需要 agent.stream()）
 * </ul>
 *
 * <h3>集成机制</h3>
 * <p>本类通过组合（has-a）使用 AgentScope API，不继承任何 AgentScope 类。
 * 所有 AAF 行为通过 Hook 接口注入 ReAct 循环，保持 ReActAgent 不被子类化。
 *
...
public class AgentScopeRuntime implements AgentRuntime {
     ...
     * 内部调用 {@link #buildReActAgent} 后包装为 {@link AgentScopeAgentAdapter}。
     */
    @Override
     ...
     * AG-UI 协议需要调用 {@code agent.stream(msg)} 产出事件流，因此返回原始类型。
     */
    ...
    private ReActAgent buildReActAgent(AgentDefinition definition) {
        var builder =
                ReActAgent.builder()
                        ...
                        .hook(new AafToolWhitelistHook(  // per-agent 实例（白名单不同）
     ...
     *
     * <p>AgentScope SkillBox 实现渐进式披露：Agent 默认只看到 skill 列表，
     * 用户明确激活某技能后才暴露该技能绑定的工具，避免工具过多干扰 LLM 决策。
     */
     ...
     * 配置模型和 Memory。
     *
     * <p>模型选择走 {@link CapabilityRouter} 六层决策链（全局默认→编排配置→用户偏好→...）。
     * Memory 使用 {@link AafAutoContextMemoryAdapter}（AutoContextMemory），
     * 按 {@code definition.memoryConfig} JSON 配置压缩参数，为 null 时使用全局默认值。
     */
    ...
    /**
     * 解析 memoryConfig JSON，覆盖默认压缩参数。
     * 为 null 或解析失败时返回全局默认配置。
     */
    private AutoContextConfig parseMemoryConfig(String json) {
        var b = AutoContextConfig.builder()
                .maxToken(100_000)
                .msgThreshold(50)
                .lastKeep(10)
        ...
        if (json != null && !json.isBlank()) {
            try {
                ...
                if (node.has("largePayloadThreshold")) b.largePayloadThreshold(node.get("largePayloadThreshold").asInt());
                if (node.has("minConsecutiveToolMessages")) b.minConsecutiveToolMessages(node.get("minConsecutiveToolMessages").asInt());
            } catch (Exception e) {
