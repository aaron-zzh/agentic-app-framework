 ...
 * {@code InvocationContext} 与 L3 的编排层同时引用。自带 {@link NodeKind} 而不复用 {@code TaskPlan.Kind}，避免 L2 反向依赖
 * L3——由编排层负责把 {@code TaskPlan.Kind} 映射进来。
 *
 * <p><b>安全约束</b>：{@code nodeId} 在动态分解模式下是**模型生成的自由字符串**—— {@code
 * TaskPlanDraft.ExecutorAssignment.nodeId} 来自协调者输出的 plan JSON，{@code decodeAndValidatePlan}
 ...
 * 定义标识，不等于"角色"。
 *
 * @param nodeId 板内节点键；动态分解时由协调者命名，仅作展示与路径
 * @param kind 节点类型
 * @param roleKey 角色键，稳定聚合维度；可空
 * @param skillKey 技能键，稳定聚合维度；可空
 */
public record NodeIdentity(
        String nodeId, NodeKind kind, String roleKey, String skillKey, boolean delivery) {
    ...
    /**
     * 编排节点类型；与 {@code TaskPlan.Kind} 一一对应，由编排层映射，避免跨层依赖。
     *
     * <p>四类 kind 保留在领域模型里（各有真实职责），下游投影只关心 {@code delivery} 这个**二分**结论：交付类节点的输出
     * 是面向用户的最终回复，内部类节点的输出是中间产物。
     */
    public enum NodeKind {
        /** 协调者：前注意、拆解、产计划，恒为内部节点。 */
        COORDINATOR,
        /** 执行者：处理分配的子任务，恒为内部节点。 */
        EXECUTOR,
        /** 评估者：有界迭代组的收敛判定者，本质是被标记的执行者，恒为内部节点。 */
        EVALUATOR,
        /** 聚合者：正式计划的 Owner finalizer，也是唯一交付者。 */
        AGGREGATOR
    }

    public NodeIdentity {
        Objects.requireNonNull(kind, "kind 不能为空");
        if (nodeId == null || !SAFE_NODE_KEY.matcher(nodeId).matches()) {
            throw new IllegalArgumentException("nodeId 必须是安全节点键（模型输出需经此校验）: " + nodeId);
        }
     ...
     *
     * <p>{@code delivery} 仍作为事件身份的一部分持久化，使 AG-UI 投影无需反查 TaskPlan。正式计划由 {@code
     * AssistantCommand.nodeIdentityOf} 一次算定：AGGREGATOR 交付，COORDINATOR / EXECUTOR / EVALUATOR
     * 均为内部节点。
     */
    ...
    public String sourcePath() {
        return userFacing() ? null : "coordinator/" + nodeId;
    }
