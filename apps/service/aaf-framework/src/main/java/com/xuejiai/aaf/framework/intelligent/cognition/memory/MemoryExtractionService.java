...
/**
 * 从对话文本中抽取结构化记忆原子和关系。
 * 被 Agent 调用时触发，属于认知层 Agentic 能力。
 */
@Slf4j
@Service
...
public class MemoryExtractionService {
     ...
     * <p>流程（对齐认知心理学编码过程）：
     * <ol>
     *   <li>注意筛选：LLM 判断哪些信息值得记忆</li>
     *   <li>结构化：提取实体、关系、标签</li>
     *   <li>编码决策：与已有记忆比对，决定 ADD/UPDATE/DELETE/NONE</li>
     *   <li>持久化：写入 AtomMemoryEngine</li>
     * </ol>
     *
     * @param userId 用户 ID
     ...
     * @return 抽取并存储的原子列表
     */
    public List<MemoryAtom> extractAndStore(Long userId, String conversationText, Instant eventTime) {
        ...
        var storedAtoms = decisions.stream()
            .filter(d -> d.action() == MemoryDeduplicationService.Action.ADD
                      || d.action() == MemoryDeduplicationService.Action.UPDATE)
            .map(d -> {
        ...
        // 5. 存储关系（仅对新增的原子建立关系）
        var addedAtoms = decisions.stream()
            .filter(d -> d.action() == MemoryDeduplicationService.Action.ADD)
            .toList();
        if (addedAtoms.size() > 1 && extraction.relations() != null) {
            for (var rel : extraction.relations()) {
                if (rel.sourceIndex() < addedAtoms.size() && rel.targetIndex() < addedAtoms.size()) {
        ...
        log.debug("记忆编码完成: {} 条决策（ADD={}, UPDATE={}, DELETE={}, NONE={}）",
            decisions.size(),
            decisions.stream().filter(d -> d.action() == MemoryDeduplicationService.Action.ADD).count(),
            decisions.stream().filter(d -> d.action() == MemoryDeduplicationService.Action.UPDATE).count(),
            decisions.stream().filter(d -> d.action() == MemoryDeduplicationService.Action.DELETE).count(),
            decisions.stream().filter(d -> d.action() == MemoryDeduplicationService.Action.NONE).count());
    ...
    private ExtractionResult callLlmExtract(String text) {
        try {
            var messages = List.of(
                (org.springframework.ai.chat.messages.Message) new SystemMessage(EXTRACTION_PROMPT),
    ...
    private ExtractionResult parseExtraction(String json) {
        try {
            ...
            var atomsRaw = objectMapper.convertValue(map.get("atoms"), new TypeReference<List<Map<String, Object>>>() {});
            var relationsRaw = objectMapper.convertValue(map.get("relations"), new TypeReference<List<Map<String, Object>>>() {});

            var atoms = atomsRaw != null ? atomsRaw.stream().map(m -> new RawAtom(
                (String) m.get("content"),
                m.get("tags") instanceof List<?> tags ? tags.stream().map(Object::toString).toList() : List.<String>of(),
                (String) m.get("scope")
            )).toList() : List.<RawAtom>of();

            var relations = relationsRaw != null ? relationsRaw.stream().map(m -> new RawRelation(
                ((Number) m.get("source_index")).intValue(),
                ((Number) m.get("target_index")).intValue(),
                (String) m.get("type"),
                m.get("weight") instanceof Number n ? n.doubleValue() : 1.0
            )).toList() : List.<RawRelation>of();
