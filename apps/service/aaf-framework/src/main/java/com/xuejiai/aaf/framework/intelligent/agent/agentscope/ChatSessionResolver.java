 ...
 *
 * <p>framework 层定义契约，api 层（ai.chat 模块）实现，桥接 ChatSession/ChatMessage。
 * 供 {@link AafAgentResolver} 在 /agui/runs 链路冷启动时设置上下文、播种历史。
 */
public interface ChatSessionResolver {

    /** 会话上下文 */
    record SessionContext(
            Long userId, String assistantId, Long knowledgeBaseId, Long sessionId) {}
