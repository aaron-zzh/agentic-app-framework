...
/**
 * 记忆管道工厂：按 MemoryStrategy 选择对应 Pipeline 实现。
 * v0.6 编排落地后可扩展为可视化节点配置。
 */
@Component
...
public class MemoryPipelineFactory {
    ...
    private record MemoryOnlyPipeline(
        ...
        UnifiedRetrievalService unifiedRetrieval
        ...
        public MemoryContext execute(PipelineInput input) {
            ...
            var result = unifiedRetrieval.retrieve(
                new UnifiedRetrievalService.RetrievalRequest(input.query(), input.userId(), null, 8)
            );
            var longTermBlock = result.memoryResults().isEmpty() ? null : result.memoryResults().toString();
            return new MemoryContext(shortTermBlock, longTermBlock, null, null, 0);
        }
    }

    /** 仅查知识库（客服/问答场景） */
    private record KnowledgeOnlyPipeline(
        UnifiedRetrievalService unifiedRetrieval
    ) implements MemoryPipeline {
        @Override
        public MemoryContext execute(PipelineInput input) {
            var result = unifiedRetrieval.retrieve(
                new UnifiedRetrievalService.RetrievalRequest(input.query(), null, input.knowledgeBaseId(), 10)
            );
            var knowledgeBlock = result.knowledgeResults().isEmpty() ? null : result.knowledgeResults().toString();
    ...
    private record ProceduralFirstPipeline(
        ...
        public MemoryContext execute(PipelineInput input) {
            ...
            var result = unifiedRetrieval.retrieve(
                new UnifiedRetrievalService.RetrievalRequest(input.query(), input.userId(), input.knowledgeBaseId(), 10)
            );
