 ...
 *
 * <p>风险等级：HIGH（涉及外部数据源访问）。 通过 HTTP API 调用独立部署的数据采集服务，AAF 侧只负责调度和权限控制。
 */
@Slf4j
@Component
public class DataCollectorTool {
    ...
    public String collect(
            ...
            @ToolParam(description = "任务类型：search/user_posts/comments/video_detail")
                    String taskType,
    ...
    public String collectStatus(@ToolParam(description = "任务ID") String taskId) {
        try {
            var request =
                    HttpRequest.newBuilder()
