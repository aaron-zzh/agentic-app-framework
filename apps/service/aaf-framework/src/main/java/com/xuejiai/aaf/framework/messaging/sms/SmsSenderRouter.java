 ...
 * <p><b>路由逻辑</b>：
 * <ol>
 *   <li>调用 {@link #sendWith(String, String, String, Map)} 时，按传入的 provider 选择厂商</li>
 *   <li>调用 {@link #send(String, String, Map)} 时，使用系统默认 provider（yaml 配置）</li>
 *   <li>模板的 {@code provider} 字段为 null 时，退回系统默认</li>
 * </ol>
 *
 * <p><b>支持的切换场景</b>：
 * <ul>
 *   <li>✅ 不同业务场景用不同厂商（如验证码用阿里云，营销短信用腾讯云）</li>
 *   <li>✅ 修改 sys_sms_template.provider 后立即生效（无需重启）</li>
 *   <li>❌ 不支持运行时修改厂商 AccessKey（需重启，如需支持需将厂商配置存数据库）</li>
 * </ul>
 *
 * <p><b>初始化</b>：启动时根据 yaml 配置初始化所有已配置厂商的 client，
 * 未配置 accessKeyId 的厂商不会注册，调用时会抛 {@link IllegalArgumentException}。
 */
@Slf4j
public class SmsSenderRouter implements SmsSender {
    ...
    /** 按指定 provider 发送 */
    public void sendWith(String provider, String phone, String templateCode, Map<String, String> params) {
