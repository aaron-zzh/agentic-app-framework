...
public final class DelegatedTaskCoordinator {
    ...
    public Flux<ExecutionEvent> dispatch(TenantId tenantId, TaskId taskId, String workerId) {
        ...
        var lease = leases.acquire(tenantId, stored.task().conversationId(), workerId, leaseTtl)
                .orElse(null);
        ...
        return withHeartbeat(executeBoard(command, parentContext, board.maxParallelism()), taskId, lease)
    ...
    public DelegatedTask stop(
            TenantId tenantId, UserId userId, TaskId taskId, String reason) {
        var stored = requireOwned(tenantId, userId, taskId);
        var lease = leases.preempt(
    ...
    public DelegatedTask takeOver(
        ...
        var stored = requireOwned(tenantId, userId, taskId);
        var lease = leases.preempt(
                tenantId, stored.task().conversationId(), "takeover-" + randomId(), leaseTtl);
        try {
            cancelRunningChildren(tenantId, taskId);
    ...
    public Mono<DelegatedTask> acceptInput(ExecutionInput input) {
        ...
        if (input.kind() == ExecutionInput.Kind.UNRELATED) {
            ...
            try {
                return appendInputEvent(stored.command().withLease(leaseUse.lease(), input.receivedAt()), input,
                                leaseUse.lease())
        ...
        var lease = leases.preempt(
                input.tenantId(), stored.task().conversationId(), "input-" + randomId(), leaseTtl);
        var released = new AtomicBoolean();
        try {
            cancelRunningChildren(input.tenantId(), input.taskId());
            boards.interruptRunning(
                    input.tenantId(), input.taskId(), input.kind() != ExecutionInput.Kind.CANCEL, lease);
            var changed = tasks.applyInput(input, lease);
            return appendInputEvent(stored.command().withLease(lease, input.receivedAt()), input, lease)
                    .map(ignored -> changed)
                    .doOnSuccess(task -> {
    ...
    public int recoverAndDispatch(String workerId, int limit) {
        ...
        tasks.findDispatchable(clock.instant(), limit)
                .forEach(stored -> dispatch.signal(stored.task().tenantId(), stored.task().taskId()));
        return recovered;
    }
    ...
    private Flux<ExecutionEvent> executeBoard(
        ...
        return Flux.defer(() -> {
            var latest = tasks.find(parentContext.tenantId(), parentContext.taskId())
                    .map(DelegatedTaskPort.StoredTask::task)
                    .orElseThrow(() -> new IllegalStateException("委托任务已丢失"));
            if (latest.status() != Status.RUNNING) return Flux.empty();
            var claim = boards.claimReady(
                    parentContext.tenantId(), parentContext.taskId(), parentContext.lease());
            if (claim.subTasks().isEmpty()) {
                return finalizeBoard(parentContext, claim.board());
            }
            return Flux.fromIterable(claim.subTasks())
                    .flatMap(subTask -> executeSubTask(parentCommand, parentContext, subTask), parallelism)
                    .concatWith(Flux.defer(() -> executeBoard(parentCommand, parentContext, parallelism)));
        });
    }

    private Flux<ExecutionEvent> executeSubTask(
            AssistantCommand parentCommand, InvocationContext parentContext, SubTask subTask) {
        var childCommand = parentCommand.forSubTask(subTask, parentCommand.lease(), clock.instant());
        ...
        return commands.execute(childCommand)
                .doOnNext(event -> {
                    ...
                    if (event.type() == ExecutionEventType.EXECUTION_COMPLETED) completed.set(true);
                    if (event.status() == ExecutionEventStatus.AWAITING_AUTHORIZATION) {
                        awaitingAuthorization.set(true);
                        tasks.awaitAuthorization(parentContext, "子任务等待工具授权", clock.instant());
                        notify(parentContext, Type.AUTHORIZATION_GAP, "任务等待授权",
                                event.eventId().value(), event.payload().values());
                    }
                ...
                .onErrorResume(error -> {
                    failure.compareAndSet(null, Objects.requireNonNullElse(error.getMessage(), "子任务失败"));
                    return Flux.empty();
                })
                .concatWith(Flux.defer(() -> {
                    if (awaitingAuthorization.get()) {
                        boards.interruptSubTask(parentContext.tenantId(), parentContext.taskId(),
                                subTask.subTaskId(), true, parentContext.lease());
                        return Flux.empty();
                    }
                    if (failure.get() != null || !completed.get()) {
                        boards.failSubTask(parentContext.tenantId(), parentContext.taskId(),
                                subTask.subTaskId(),
                                Objects.requireNonNullElse(failure.get(), "子任务未产生完整终态"),
                                failure.get() == null || isTransientMessage(failure.get()),
                                parentContext.lease());
                    } else {
                        boards.completeSubTask(parentContext.tenantId(), parentContext.taskId(),
                                subTask.subTaskId(), result.get(), parentContext.lease());
                    }
                    return Flux.empty();
                }));
    }
    ...
    private Flux<ExecutionEvent> finalizeBoard(InvocationContext context, TaskBoard board) {
        ...
        var paused = tasks.pause(
                context.tenantId(), context.taskId(), context.lease(),
                "TaskBoard 无可运行节点", clock.instant());
    ...
    private Flux<ExecutionEvent> withHeartbeat(
        ...
        return execution.publish(shared -> Flux.merge(
                ...
                Flux.interval(period)
                        .doOnNext(ignored -> {
                            var renewed = leases.renew(currentLease.get(), leaseTtl)
    ...
    private void cancelRunningChildren(TenantId tenantId, TaskId taskId) {
        boards.find(tenantId, taskId).ifPresent(board -> board.subTasks().values().stream()
                .filter(subTask -> subTask.status() == TaskBoard.Status.RUNNING)
                .forEach(subTask -> agentExecution.cancel(subTask.executionId()).subscribe()));
    }

    private void failParent(InvocationContext context, Throwable failure) {
        try {
            var failed = tasks.failOrRetry(
                    context,
    ...
    private void notifyDeadlinePauseIfNeeded(TenantId tenantId, TaskId taskId) {
        tasks.find(tenantId, taskId).map(DelegatedTaskPort.StoredTask::task)
                .filter(latest -> latest.status() == Status.PAUSED)
                .ifPresent(latest -> notifyStatus(
                        latest, "委托任务已到 deadline 并暂停", "deadline-paused"));
    }

    private void notifyBudgetIfNeeded(InvocationContext context) {
        var task = tasks.find(context.tenantId(), context.taskId())
                .map(DelegatedTaskPort.StoredTask::task).orElse(null);
        if (task == null || !task.contract().notificationPolicy().triggers()
        ...
        var warning = (double) usage.modelTokens() / budget.modelTokens() >= ratio
                || (double) usage.toolUnits() / budget.toolUnits() >= ratio
                || usage.credits().divide(budget.credits(), 4, java.math.RoundingMode.HALF_UP)
                        .doubleValue() >= ratio;
        if (warning) {
    ...
    private void notifyFailure(DelegatedTask task, String failure) {
        if (task.contract().notificationPolicy().triggers()
                .contains(NotificationTrigger.CONSECUTIVE_FAILURE)) {
            notifications.notify(new Notification(
                    notificationId(task.taskId(), "failure-" + task.consecutiveFailures()),
                    task.tenantId(), task.userId(), task.taskId(), Type.CONSECUTIVE_FAILURE,
    ...
    private void notifyStatus(DelegatedTask task, String summary, String suffix) {
        ...
        notifications.notify(new Notification(
                ...
                task.tenantId(), task.userId(), task.taskId(),
                completed ? Type.COMPLETED : Type.STATUS_CHANGED,
                summary, Map.of("status", task.status().name()), clock.instant()));
    }
    ...
    private void notify(
        ...
        notifications.notify(new Notification(
                notificationId(context.taskId(), suffix), context.tenantId(), context.userId(),
                context.taskId(), type, summary, details, clock.instant()));
    }

    private Mono<ExecutionEvent> appendInputEvent(
            AssistantCommand command, ExecutionInput input, Lease lease) {
        var type = switch (input.kind()) {
        ...
        var event = new ExecutionEvent(
                new EventId("input-" + input.inputId()), command.tenantId(), command.conversationId(),
                command.sessionId(), command.taskId(), command.executionId(), command.runId(),
                command.parentExecutionId(), 0, type, status, command.controlMode(), OwnerType.HUMAN,
                command.assistantId(), null, command.userId(), command.correlationId(),
                command.causationId(), command.idempotencyKey(), new ExecutionEventPayload(payload),
                input.receivedAt());
    ...
    private InvocationContext context(AssistantCommand command) {
        return new InvocationContext(
                command.tenantId(), command.userId(), command.assistantId(), command.conversationId(),
                command.sessionId(), command.taskId(), command.executionId(), command.runId(),
                command.parentExecutionId(), command.correlationId(), command.causationId(),
                command.idempotencyKey(), command.controlMode(), command.executionContract(),
                command.lease(), new ToolAuthorizationContext(Map.of()));
    }
