...
@Entity
@Table(
        name = "ai_automation_run",
        uniqueConstraints = {
            @UniqueConstraint(columnNames = {"tenant_id", "automation_id", "trigger_key"}),
            @UniqueConstraint(columnNames = {"tenant_id", "run_id"})
        })
public class AutomationRunEntity {
    ...
    @Column(name = "automation_id", nullable = false, length = 128)
    ...
    @Column(name = "run_payload", nullable = false, columnDefinition = "jsonb")
