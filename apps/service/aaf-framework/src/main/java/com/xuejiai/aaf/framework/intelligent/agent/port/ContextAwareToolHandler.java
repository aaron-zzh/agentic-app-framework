 ...
 *
 * <p><b>治理元数据自声明（对齐官方 {@code AgentTool}/{@code ToolBase} 契约）</b>：内置系统工具随代码发布， {@code
 * readOnly}/{@code reversible}/{@code inputSchema} 是代码固有属性，不是运营方需要在后台配置的可变数据—— 与 Connector/MCP
 * 类工具（回调运行时动态注册、权限确需运营方随时调整）性质不同。核实官方 <a
 * href="https://java.agentscope.io/v2/zh/docs/building-blocks/tool.html">Tool 文档</a> 后确认，官方 {@code
 * AgentTool}/{@code ToolBase} 本身就要求工具自己声明 {@code isReadOnly()}/{@code getParameters()}，
 * 不依赖外部数据库治理。{@code AgentScopeToolkitFactory} 对内置工具（实现本接口）优先使用这里声明的元数据， 不再强制查询 {@code
 * ai_tool_catalog}；只有 Connector/MCP 类工具才继续走目录治理。
 *
...
public interface ContextAwareToolHandler {
    ...
    /**
     * 是否可撤销的写操作。与 {@code readOnly} 互斥（不能同时为 {@code true}，对齐 {@code ToolCatalogPort.ToolDefinition}
     * 的不变量）。默认 {@code false}。
     */
