...
public class ChunkQualityEvaluator {
    ...
    public ChunkQualityReport evaluate(List<DocumentChunk> chunks) {
        ...
        double avgTokenCount =
                chunks.stream().mapToInt(DocumentChunk::tokenCount).average().orElse(0);

        // 均匀度评分：基于变异系数（CV），CV 越小越均匀
        double variance =
                java.util.Arrays.stream(lengths).mapToDouble(l -> Math.pow(l - avgLength, 2)).sum()
                        / totalChunks;
        ...
        if (avgLength >= MIN_REASONABLE_LENGTH && avgLength <= MAX_REASONABLE_LENGTH) {
            ...
            rangeScore =
                    Math.max(0, 1.0 - (avgLength - MAX_REASONABLE_LENGTH) / MAX_REASONABLE_LENGTH);
        }
        ...
        return new ChunkQualityReport(
                avgLength, minLength, maxLength, totalChunks, avgTokenCount, qualityScore);
    }
}
