 ...
 * </ul>
 *
 * <p>推荐调用方通过 {@link com.xuejiai.aaf.framework.intelligent.core.model.CapabilityRouter} 获取 {@link
 * AiModel}，再调用 {@link #getSyncService(AiModel)} 取实现，最后调用接口方法。 这与 OcrController 的模式一致，确保用户偏好 /
 * 系统默认等决策链正确生效。
 *
...
public class ImageServiceFactory {
    ...
    private final MidjourneyAsyncImageService midjourneyAsyncService;
    ...
    /** 获取异步图像生成服务（按 modelId 的 providerType 路由）。 */
    public AsyncImageGenerationService getAsyncService(String modelId) {
        if (modelId == null) return dashScopeAsyncService;
        try {
            var model = modelManagementService.getModel(modelId);
            if (model.effectiveProviderType() == AiModelProviderType.MIDJOURNEY) {
                return midjourneyAsyncService;
            }
        ...
        return switch (providerType) {
            ...
            default ->
                    throw new IllegalArgumentException(
                            "模型 " + model.getModelId() + " 的协议类型 " + providerType + " 不支持图像生成");
        };
    }
     ...
     *
     * <p>注意：此方法跳过用户偏好/系统默认决策链，仅适用于已明确 modelId 的场景（如 AigcTaskExecutor）。 其他场景请先通过 CapabilityRouter 解析
     * AiModel，再调用 {@link #getSyncService(AiModel)}。
     *
