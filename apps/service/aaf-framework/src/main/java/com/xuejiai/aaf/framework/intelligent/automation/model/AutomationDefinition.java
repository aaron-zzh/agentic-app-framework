...
public record AutomationDefinition(
    ...
    public AutomationDefinition publish(Instant at) {
        if (!previewPassed || !dryRunPassed) throw new IllegalStateException("发布前必须通过 preview 与 dry-run");
    ...
    private AutomationDefinition copy(Lifecycle nextLifecycle, boolean nextEnabled, boolean preview, boolean dryRun, Instant at) {
        return new AutomationDefinition(tenantId, automationId, name, version, sourceTaskId, template,
                parameterSchema, trigger, permissionSnapshot, failurePolicy, notificationPlan,
                nextLifecycle, nextEnabled, preview, dryRun, createdAt, at);
    }
    ...
    public record TaskTemplate(
        ...
        public TaskTemplate {
            ...
            contextCandidates = List.copyOf(Objects.requireNonNull(contextCandidates, "contextCandidates 不能为空"));
            ...
            if (executionWindow.isNegative() || executionWindow.isZero()) throw new IllegalArgumentException("executionWindow 必须大于 0");
        }
    }
    ...
    public record ParameterSchema(Map<String, String> properties, Set<String> required) {
        public ParameterSchema {
            ...
            if (!properties.keySet().containsAll(required)) throw new IllegalArgumentException("required 必须属于 properties");
        }
        public void validate(Map<String, Object> parameters) {
            ...
            var unknown = values.keySet().stream().filter(key -> !properties.containsKey(key)).toList();
    ...
    public record TriggerSpec(TriggerType type, String cron, Duration frequency) {
        public TriggerSpec {
            ...
            if (type == TriggerType.CRON && (cron == null || cron.isBlank())) throw new IllegalArgumentException("CRON 必须声明 cron");
    ...
    public record PermissionSnapshot(Set<String> actions, String policyVersion, Instant capturedAt) {
    ...
    public record FailurePolicy(int maxAttempts, Duration backoff, FailureAction exhaustedAction) {
        public FailurePolicy {
            ...
            if (maxAttempts < 1 || backoff.isZero() || backoff.isNegative()) throw new IllegalArgumentException("失败策略无效");
        }
    }
    ...
    public record NotificationPlan(Set<String> channels, Set<String> events) {
        public NotificationPlan {
            ...
            if (channels.isEmpty() || events.isEmpty()) throw new IllegalArgumentException("通知渠道与事件不能为空");
        }
    }

    public enum TriggerType { MANUAL, CRON, FREQUENCY }
    public enum FailureAction { PAUSE, DISABLE, NOTIFY_OWNER }
    public enum Lifecycle { DRAFT, PUBLISHED, DEPRECATED, DISABLED }
