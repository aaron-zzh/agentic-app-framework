 ...
 *
 * <p>由内置技能 "tool_generation" 关联的 Agent 调用。
 * 生成后默认 PRIVATE（仅创建者可见），管理员/用户可标记为 SHARED。
 */
@Slf4j
@Component
...
public class ToolGenerator {
    ...
    public String generateTool(String description) {
        ...
        return "工具生成成功：\n- 名称: %s\n- 描述: %s\n- 参数: %s\n\n源码:\n```javascript\n%s\n```\n\n请用户确认后调用 confirm_tool 注册。"
                .formatted(blueprint.getName(), blueprint.getDescription(), blueprint.getParameters(), blueprint.getCode());
    }
