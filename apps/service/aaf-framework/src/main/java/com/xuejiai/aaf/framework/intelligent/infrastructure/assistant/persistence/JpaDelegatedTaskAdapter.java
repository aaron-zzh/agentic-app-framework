...
public final class JpaDelegatedTaskAdapter implements DelegatedTaskPort {
    ...
    public StoredTask create(DelegatedTask task, AssistantCommand command, TaskBoard board) {
        ...
        if (repository.findByTenantIdAndTaskId(task.tenantId().value(), task.taskId().value()).isPresent()
                || taskBoards.findByTenantIdAndTaskId(task.tenantId().value(), task.taskId().value()).isPresent()) {
            throw new IllegalStateException("委托任务已存在: " + task.taskId().value());
        }
        if (!task.tenantId().equals(command.tenantId()) || !task.taskId().equals(command.taskId())
                || !task.taskId().equals(board.taskId()) || command.executionContract() == null) {
    ...
    public Optional<StoredTask> find(TenantId tenantId, TaskId taskId) {
        return repository.findByTenantIdAndTaskId(tenantId.value(), taskId.value()).map(this::stored);
    }
    ...
    public List<StoredTask> findDispatchable(Instant now, int limit) {
        if (limit < 1 || limit > 100) throw new IllegalArgumentException("dispatch limit 必须在 1..100");
        return repository.findDispatchable(now, PageRequest.of(0, limit)).stream().map(this::stored).toList();
    }
    ...
    public Optional<StoredTask> claim(TenantId tenantId, TaskId taskId, Lease lease, Instant now) {
        ...
        if (current.status() != Status.PENDING || current.nextRunAt().isAfter(now)
        ...
        if (!current.contract().deadline().isAfter(now)) {
            var paused = copy(current, Status.PAUSED, current.owner(), current.budgetUsage(),
                    current.attempts(), current.consecutiveFailures(), current.nextRunAt(),
                    null, null, current.fencingToken(),
                    merge(current.checkpoint(), "deadlineReached", now.toString()), now,
                    current.sessionId(), current.executionId());
            save(entity, paused);
            return Optional.empty();
        }
        var claimed = copy(current, Status.RUNNING, current.owner(), current.budgetUsage(),
                current.attempts() + 1, current.consecutiveFailures(), current.nextRunAt(),
                lease.ownerId(), lease.expiresAt(), lease.fencingToken(), current.checkpoint(), now,
                current.sessionId(), current.executionId());
    ...
    public DelegatedTask renew(TaskId taskId, Lease lease, Instant now) {
        ...
        var renewed = copy(current, current.status(), current.owner(), current.budgetUsage(),
                current.attempts(), current.consecutiveFailures(), current.nextRunAt(),
                lease.ownerId(), lease.expiresAt(), lease.fencingToken(), current.checkpoint(), now,
                current.sessionId(), current.executionId());
    ...
    public void requireAgentExecution(InvocationContext context) {
        ...
        if (context.controlMode()
                != com.xuejiai.aaf.framework.intelligent.shared.event.ExecutionEvent.ControlMode.DELEGATED) return;
        leases.requireCurrent(context.lease());
        var entity = repository.findByTenantIdAndTaskId(
    ...
    public void requireExecution(
        ...
        leases.requireCurrent(lease);
        var entity = repository.findByTenantIdAndTaskId(tenantId.value(), taskId.value())
                .orElseThrow(() -> stale("委托任务不存在"));
    ...
    public DelegatedTask reserveModelCall(InvocationContext context, Instant at) {
        ...
        var next = new BudgetUsage(usage.modelCalls() + 1, usage.modelTokens(), usage.toolCalls(),
                usage.toolUnits(), usage.credits());
    ...
    public DelegatedTask recordModelUsage(
        ...
        requireWithinDeadline(entity, current, at);
        var usage = current.budgetUsage();
        var next = new BudgetUsage(usage.modelCalls(), usage.modelTokens() + tokens, usage.toolCalls(),
                usage.toolUnits(), usage.credits().add(credits));
        var limit = current.contract().budget();
        if (next.modelTokens() > limit.modelTokens() || next.credits().compareTo(limit.credits()) > 0) {
    ...
    public DelegatedTask reserveToolCall(InvocationContext context, String action, Instant at) {
        ...
        var usage = current.budgetUsage();
        var next = new BudgetUsage(usage.modelCalls(), usage.modelTokens(), usage.toolCalls() + 1,
                usage.toolUnits(), usage.credits());
    ...
    public DelegatedTask recordToolUsage(
        ...
        requireWithinDeadline(entity, current, at);
        var usage = current.budgetUsage();
        var next = new BudgetUsage(usage.modelCalls(), usage.modelTokens(), usage.toolCalls(),
                usage.toolUnits() + units, usage.credits().add(credits));
        var limit = current.contract().budget();
    ...
    public DelegatedTask checkpoint(
        ...
        var entity = locked(context);
        var current = entity.getTask();
        var changed = copy(current, current.status(), current.owner(), current.budgetUsage(),
                current.attempts(), current.consecutiveFailures(), nextRunAt,
                current.leaseOwner(), current.leaseUntil(), current.fencingToken(), checkpoint, at,
                current.sessionId(), current.executionId());
    ...
    public DelegatedTask pause(
        ...
        var paused = copy(current, Status.PAUSED, current.owner(), current.budgetUsage(),
                current.attempts(), current.consecutiveFailures(), current.nextRunAt(),
                null, null, lease.fencingToken(), checkpoint, at, current.sessionId(), current.executionId());
        return save(entity, paused);
    }

    @Override
    @Transactional
    public DelegatedTask awaitAuthorization(
            InvocationContext context, String reason, Instant at) {
        var entity = locked(context);
        var current = entity.getTask();
        var waiting = copy(current, Status.AWAITING_AUTHORIZATION, current.owner(),
                current.budgetUsage(), current.attempts(), current.consecutiveFailures(),
                current.nextRunAt(), null, null, current.fencingToken(),
                merge(current.checkpoint(), "authorizationGap", reason), at,
                current.sessionId(), current.executionId());
    ...
    public DelegatedTask resumeAfterAuthorization(
        ...
        var resumed = copy(current, Status.PENDING, current.owner(), current.budgetUsage(),
                current.attempts(), current.consecutiveFailures(), at, null, null,
                lease.fencingToken(), current.checkpoint(), at,
                current.sessionId(), current.executionId());
    ...
    @Override
    @Transactional
    public DelegatedTask complete(InvocationContext context, Map<String, Object> result, Instant at) {
        var entity = locked(context);
        var current = entity.getTask();
        var completed = copy(current, Status.COMPLETED, current.owner(), current.budgetUsage(),
                current.attempts(), 0, current.nextRunAt(), null, null, current.fencingToken(),
                result, at, current.sessionId(), current.executionId());
    ...
    public DelegatedTask fail(InvocationContext context, String failure, Instant at) {
        ...
        return save(entity, copy(current, Status.FAILED, current.owner(), current.budgetUsage(),
                current.attempts(), current.consecutiveFailures() + 1, current.nextRunAt(),
                null, null, current.fencingToken(), checkpoint, at,
                current.sessionId(), current.executionId()));
    }
    ...
    public DelegatedTask failOrRetry(
        ...
        var retry = current.attempts() < policy.maxAttempts()
                && (!policy.retryTransientOnly() || transientFailure)
                && current.contract().deadline().isAfter(at.plus(policy.initialBackoff()));
        var checkpoint = merge(current.checkpoint(), "lastFailure", failure);
        if (!retry) {
            return save(entity, copy(current, Status.FAILED, current.owner(), current.budgetUsage(),
                    current.attempts(), failures, current.nextRunAt(), null, null,
                    current.fencingToken(), checkpoint, at, current.sessionId(), current.executionId()));
        }
        ...
        entity.setCommand(entity.getCommand().newExecution(
                nextExecution, nextSession, nextRun, context.lease(), at));
        return save(entity, copy(current, Status.PENDING, new Owner(OwnerKind.ASSISTANT,
                        entity.getCommand().assistantId().value()), current.budgetUsage(),
                current.attempts(), failures, nextAt, null, null, current.fencingToken(), checkpoint,
                at, nextSession, nextExecution));
    }
    ...
    public DelegatedTask cancel(
        ...
        return save(entity, copy(current, Status.CANCELED, new Owner(OwnerKind.HUMAN, userId.value()),
                current.budgetUsage(), current.attempts(), current.consecutiveFailures(),
                current.nextRunAt(), null, null, lease.fencingToken(),
                merge(current.checkpoint(), "cancelReason", reason), at,
                current.sessionId(), current.executionId()));
    }
    ...
    public DelegatedTask takeOver(
        ...
        if (current.contract().takeoverPolicy().reasonRequired() && (reason == null || reason.isBlank())) {
            throw new IllegalArgumentException("人工接管必须说明原因");
        }
        return save(entity, copy(current, Status.PAUSED, new Owner(OwnerKind.HUMAN, userId.value()),
                current.budgetUsage(), current.attempts(), current.consecutiveFailures(),
                current.nextRunAt(), null, null, lease.fencingToken(),
                merge(current.checkpoint(), "takeoverReason", reason), at,
                current.sessionId(), current.executionId()));
    }
    ...
    public StoredTask handBack(
            TenantId tenantId, UserId userId, TaskId taskId, ExecutionId executionId,
            SessionId sessionId, Lease lease, Instant at) {
        leases.requireCurrent(lease);
        var entity = requireOwned(tenantId, userId, taskId);
        var current = entity.getTask();
        ...
        entity.setCommand(entity.getCommand().newExecution(executionId, sessionId, runId, lease, at));
        var returned = copy(current, Status.PENDING,
                new Owner(OwnerKind.ASSISTANT, entity.getCommand().assistantId().value()),
                current.budgetUsage(), current.attempts(), current.consecutiveFailures(), at,
                null, null, lease.fencingToken(), current.checkpoint(), at, sessionId, executionId);
    ...
    public DelegatedTask applyInput(ExecutionInput input, Lease lease) {
        ...
        var checkpoint = merge(current.checkpoint(), "input:" + input.inputId(),
                Map.of("kind", input.kind().name(), "content", input.content(),
                        "receivedAt", input.receivedAt().toString()));
        if (input.kind() == ExecutionInput.Kind.CANCEL) {
            return save(entity, copy(current, Status.CANCELED,
                    new Owner(OwnerKind.HUMAN, input.userId().value()), current.budgetUsage(),
                    current.attempts(), current.consecutiveFailures(), input.receivedAt(),
                    null, null, lease.fencingToken(), checkpoint, input.receivedAt(),
                    current.sessionId(), current.executionId()));
        }
        var nextExecution = new ExecutionId(randomId());
        var nextSession = new SessionId(randomId());
        var nextRun = new RunId(randomId());
        entity.setCommand(entity.getCommand()
                .newExecution(nextExecution, nextSession, nextRun, lease, input.receivedAt())
                .withInput(input.content(), lease, input.receivedAt()));
        var nextCheckpoint = merge(checkpoint, "resumeInput", input.content());
        return save(entity, copy(current, Status.PENDING,
                new Owner(OwnerKind.ASSISTANT, entity.getCommand().assistantId().value()),
                current.budgetUsage(), current.attempts(), current.consecutiveFailures(),
                input.receivedAt(), null, null, lease.fencingToken(), nextCheckpoint,
                input.receivedAt(), nextSession, nextExecution));
    }
    ...
    public int recoverExpired(Instant now) {
        ...
        expired.forEach(entity -> {
            var current = entity.getTask();
            var recovered = copy(current, Status.PENDING, current.owner(), current.budgetUsage(),
                    current.attempts(), current.consecutiveFailures(), now, null, null,
                    current.fencingToken(), merge(current.checkpoint(), "recoveredAt", now.toString()),
                    now, current.sessionId(), current.executionId());
    ...
    private DelegatedTask requireCurrent(
        ...
        if (task.status() != Status.RUNNING || task.owner().kind() == OwnerKind.HUMAN
                || task.fencingToken() != lease.fencingToken()
                || !Objects.equals(task.leaseOwner(), lease.ownerId())
                || (executionId != null && !task.executionId().equals(executionId)
    ...
    private boolean isRunningChild(TenantId tenantId, TaskId taskId, ExecutionId executionId) {
        return taskBoards.findByTenantIdAndTaskId(tenantId.value(), taskId.value())
                .map(TaskBoardEntity::getBoard)
                .stream()
                .flatMap(board -> board.subTasks().values().stream())
                .anyMatch(subTask -> subTask.status()
                                == com.xuejiai.aaf.framework.intelligent.assistant.model.TaskBoard.Status.RUNNING
    ...
    private DelegatedTask pauseAndThrow(
            DelegatedTaskEntity entity, DelegatedTask current, BudgetUsage usage,
            String reason, Instant at) {
        var paused = copy(current, Status.PAUSED, current.owner(), usage, current.attempts(),
                current.consecutiveFailures(), current.nextRunAt(), null, null,
                current.fencingToken(), merge(current.checkpoint(), "budgetPause", reason), at,
                current.sessionId(), current.executionId());
        save(entity, paused);
    ...
    private DelegatedTask withUsage(DelegatedTask current, BudgetUsage usage, Instant at) {
        return copy(current, current.status(), current.owner(), usage, current.attempts(),
                current.consecutiveFailures(), current.nextRunAt(), current.leaseOwner(),
                current.leaseUntil(), current.fencingToken(), current.checkpoint(), at,
                current.sessionId(), current.executionId());
    }
    ...
    private DelegatedTaskEntity requireForUpdate(TenantId tenantId, TaskId taskId) {
        return repository.findForUpdate(tenantId.value(), taskId.value())
    ...
    private static DelegatedTask copy(
            DelegatedTask source, Status status, Owner owner, BudgetUsage usage,
            int attempts, int failures, Instant nextRunAt, String leaseOwner,
            Instant leaseUntil, long fencingToken, Map<String, Object> checkpoint,
