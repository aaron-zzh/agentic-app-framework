 ...
 *   <li>需要推理 → 选推理模型（deepseek-reasoner / o1）
...
public class AiModelSelector {
     ...
     */
    public String select(ModelRoutingContext ctx) {
        if (ctx.taskFeatures() == null || ctx.taskFeatures().isEmpty()) return null;

        var features = ctx.taskFeatures();
        var capability = ctx.capability() != null ? ctx.capability() : "CHAT";

        // 含图片/视频 → 需要 VISION 能力
        if (Boolean.TRUE.equals(features.get(ModelRoutingContext.FEATURE_HAS_IMAGE))
                || Boolean.TRUE.equals(features.get(ModelRoutingContext.FEATURE_HAS_VIDEO))) {
        ...
        // 需要推理 → 选推理模型
        if (Boolean.TRUE.equals(features.get(ModelRoutingContext.FEATURE_REASONING_REQUIRED))) {
            return modelRepository.findByEnabledTrueOrderBySortOrder().stream()
        ...
        // 长文本 → 选大上下文窗口模型
        var inputLength = (Number) features.get(ModelRoutingContext.FEATURE_INPUT_LENGTH);
        ...
        // 成本敏感 → 选最便宜的
        if (Boolean.TRUE.equals(features.get(ModelRoutingContext.FEATURE_COST_SENSITIVE))) {
            return modelRepository.findByEnabledTrueOrderBySortOrder().stream()
                    .filter(m -> m.hasCapability(capability))
