 ...
 *
 * <p>两者作用对象与生命周期不同：{@code ContextCompressionPort} 是单次 Harness 调用前的全局 Prompt
 * 预算治理，随执行画像冻结；本端口只处理被挤出短期记忆最近窗口的旧会话轮次， 跨多次物理调用增量维护，失败时降级为保留旧摘要，不阻断当前执行。
 */
public interface SessionContextCompressionPort {
     ...
     * 对被挤出窗口的旧轮次生成结构化摘要。
     *
     * <p>调用方必须保证 {@code messagesToSummarize} 非空且已按时间正序排列；失败或超时时抛出异常， 调用方按"保留旧摘要 +
     * 最新原文尾部"降级，不得阻断主执行。
     *
