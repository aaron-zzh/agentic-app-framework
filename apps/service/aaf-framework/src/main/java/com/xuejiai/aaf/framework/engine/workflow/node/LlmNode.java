 ...
 * <ul>
 *   <li>input（必填）——用户输入</li>
 *   <li>systemPrompt（可选）——系统提示词</li>
 *   <li>scene（可选，默认 CHAT）——场景标识</li>
 *   <li>userId（可选）——用户 ID</li>
 *   <li>modelId（可选）——指定模型标识</li>
 *   <li>temperature（可选）——温度参数</li>
 *   <li>maxTokens（可选）——最大 Token 数</li>
 *   <li>outputSchema（可选）——JSON Schema 字符串，启用结构化输出</li>
 *   <li>output/success（节点写入）</li>
 * </ul>
 */
@Slf4j
@Component("llmNode")
...
public class LlmNode implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) {
        ...
        var scene = execution.getVariable("scene") != null
                ? (String) execution.getVariable("scene") : "CHAT";
        var userId = execution.getVariable("userId") != null
                ? ((Number) execution.getVariable("userId")).longValue() : null;
        var modelId = (String) execution.getVariable("modelId");
        var outputSchema = (String) execution.getVariable("outputSchema");

        // 温度参数
        var temperature = execution.getVariable("temperature") != null
                ? ((Number) execution.getVariable("temperature")).doubleValue() : 0.9;
        ...
        var request = new ExecutionRequest(
                ExecutionTarget.LLM, targetId, input,
                null, userId, effectivePrompt, temperature, true);
