...
/**
 * Assistant 会话处理入口：
 * 用户输入 → 意图理解 → Skill 匹配 → Agent 认知循环 → 记忆更新 → 返回。
 */
@Slf4j
@Service
...
public class AssistantService {
     ...
     * @return 助理响应
     */
    public AssistantResponse handle(String sessionId, Long userId, String assistantId, String userInput) {
        // 1. 会话管理
        ...
        // 3. 意图理解
        var history = shortTermMemory.getAll(sessionId).stream()
            .map(MemoryMessage::content).toList();
        ...
        if (agentId != null) {
            ...
            if (definition != null) {
                ...
                skill.ifPresent(s -> {
                    if (s.getSystemPrompt() != null) definition.setSystemPrompt(s.getSystemPrompt());
                });
        ...
        return new AssistantResponse(response, intent.getIntent(),
            skill.map(s -> s.getName()).orElse(null), success);
    }

    /** 助理响应 */
    public record AssistantResponse(String content, String intent, String skillUsed, boolean success) {}
}
