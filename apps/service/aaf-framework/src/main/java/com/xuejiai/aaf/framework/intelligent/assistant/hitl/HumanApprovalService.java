...
public class HumanApprovalService {
    ...
    /** 按 sessionId 批量归档审批（AG-UI confirm 场景：不知 requestId，按会话匹配）。 */
    public void resolveBySession(String sessionId, Long userId, Decision decision, String reason) {
        pending.values().stream()
                .filter(r -> sessionId.equals(r.sessionId()))
                .map(ApprovalRequest::requestId)
                .toList()
                .forEach(id -> resolve(id, userId, decision, reason));
    }
