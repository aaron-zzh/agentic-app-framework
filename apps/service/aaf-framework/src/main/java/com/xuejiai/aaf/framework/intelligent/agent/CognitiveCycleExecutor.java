...
/**
 * Agent 认知循环执行器：包装 AgentScope ReActAgent，
 * 在执行前后注入 AAF 认知能力（记忆检索/写入）。
 *
 * <pre>
...
public class CognitiveCycleExecutor {
    ...
    public CycleResult execute(AgentDefinition definition, String input, Long userId) {
        ...
        try {
            ...
            checkpointService.saveCheckpoint(executionId, 1, buildState(executionId, agentId, "perceive"));

            // 2. 规划 + 执行（Plan + Act）：带重试
            var response = checkpointService.executeWithRetry(executionId, attempt -> {
                var agent = agentFactory.create(definition);
                var msg = Msg.builder().name("user").textContent(input).build();
                return sandbox.execute(agent, msg, Duration.ofSeconds(definition.getTimeoutSeconds())).block();
            });

            checkpointService.saveCheckpoint(executionId, 2, buildState(executionId, agentId, "execute"));

            // 3. 评估（Evaluate）
            var success = response != null && !response.getTextContent().contains("执行超时")
                && !response.getTextContent().contains("执行异常");

            // 4. 学习（Learn）：将对话写入记忆系统
            if (userId != null && success) {
                var conversationText = "用户: %s\n助手: %s".formatted(input, response.getTextContent());
    ...
    private AgentCheckpointService.ExecutionState buildState(String executionId, String agentId, String step) {
