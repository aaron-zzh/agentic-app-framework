...
public class DefaultConfidenceGate implements ConfidenceGate {
    ...
    public GateDecision evaluate(GateInput input) {
        ...
        if (input.verifiable()) {
            return new GateDecision(Action.EXECUTE_AND_VERIFY, "置信度较低，将执行并自动验证，失败时回滚。", true);
        }
        return new GateDecision(Action.PAUSE_FOR_HUMAN, "置信度较低且无法自动验证，需要人工确认后再执行。", true);
    }
}
