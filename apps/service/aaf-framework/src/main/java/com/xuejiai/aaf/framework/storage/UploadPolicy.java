 ...
 *
 * <p>修复前的问题：只有 {@code MultipartFile} 入口做了 MIME 与大小校验，URL / byte[] / Base64 三个入口
 * 以及直接调用 {@link StorageService#upload} 的代码可完全绕过校验；且默认白名单包含 {@code image/svg+xml}
 * 与 {@code text/html}，在同域对象访问下可形成存储型 XSS，超大远程文件也没有读取上限。
 *
 * <p>现在：
 *
 * <ul>
 ...
 *   <li>{@link StorageService} 各实现在 upload 入口调用静态 {@link #assertNotActiveContent} 兜底，
 *       即使有人绕过 FileService 直调存储层也拦得住
