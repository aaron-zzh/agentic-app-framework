...
public class RagGenerationService {
    ...
    public RagResponse generate(String question, Long knowledgeBaseId) {
        // 检索
        var sources = hybridSearchService.search(question, knowledgeBaseId, new HybridSearchConfig());

        // 构建上下文
        var context = IntStream.range(0, sources.size())
                .mapToObj(i -> "[%d] %s".formatted(i + 1, sources.get(i).content()))
                .collect(Collectors.joining("\n\n"));

        // 调用 LLM
        var response = ChatClient.create(chatModel).prompt()
