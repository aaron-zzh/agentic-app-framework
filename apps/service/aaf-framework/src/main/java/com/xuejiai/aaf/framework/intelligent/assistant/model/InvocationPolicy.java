...
public enum InvocationPolicy {
    ...
    COORDINATOR(
            ...
            "只使用提供的摘要引用识别不可替代阻塞项并拆分任务；不得调用业务工具、生成最终业务内容、"
                    ...
                    + "executors 必须为 1..8；aggregationContract.kind 仅允许 PASS_THROUGH、"
                    + "ORDERED_CONCAT 或 COORDINATOR_REDUCE；串行节点通过 dependsOn 与 inputBindings "
                    + "显式引用前序结果；iterationGroup 禁止依赖回边。"),
