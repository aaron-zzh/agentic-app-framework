 ...
 *
 * <p>适配策略：委托给 AgentScope {@link Memory}， 替换 AAF 自研的 {@code WorkingMemoryImpl}（Agent 执行期临时上下文）。
 *
 * <p>AgentScope 提供多种 Memory 后端（agentscope-extensions-autocontext-memory 扩展）：
 *
 * <ul>
 *   <li>{@code InMemoryMemory} — 进程内，开发/测试
 *   <li>{@code RedisMemory} — Redis 持久化，生产推荐
 *   <li>{@code SqlAlchemyMemory} — 数据库持久化
 *   <li>{@code TablestoreMemory} — 阿里云 Tablestore
 * </ul>
 *
 * <p>autocontext-memory 扩展还提供 Token 预算自动截断（P0-P5 优先级）， 解决 AAF 上下文管理器缺失的 Token 预算控制问题。
 *
...
public class AgentScopeMemoryAdapter {
    ...
    /** 获取消息数量 */
    public int size() {
        return delegate.getMessages().size();
    }
}
