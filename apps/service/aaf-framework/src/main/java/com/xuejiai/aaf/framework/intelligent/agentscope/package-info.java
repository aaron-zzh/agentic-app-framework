 ...
 *
 * <p>领域层（{@code core} 接口 + {@code agent}/{@code assistant}/{@code cognition} 根）保持零
 * AgentScope 依赖；本包是 ports-and-adapters 的 adapter：实现 AAF 接口并委托 AgentScope，
 * 或反向实现 AgentScope SPI 把自有引擎插入其扩展点。切换底层框架只需替换本包。
 *
