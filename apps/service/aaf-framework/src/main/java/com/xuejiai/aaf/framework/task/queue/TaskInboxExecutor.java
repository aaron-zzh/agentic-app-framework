...
/** 队列任务事务性收件箱，原子提交消费完成记录与 handler 的关系库副作用。 */
@Component
...
public class TaskInboxExecutor {
     ...
     * 在收件箱事务中执行 handler。
     *
     * @return true 表示本次执行了 handler，false 表示任务已完成并跳过重复执行
     */
    @Transactional
    public boolean execute(String taskId, String taskType, String payload, Runnable handler) {
        ...
        if (inserted == 0) {
            var existing =
                    jdbcTemplate.queryForObject(
                            """
                            SELECT task_type, payload_digest
                            FROM sys_task_inbox
                            WHERE task_id = ?
                            """,
                            (resultSet, rowNum) ->
    ...
    private record InboxRecord(String taskType, String payloadDigest) {}
}
