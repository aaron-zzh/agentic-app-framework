...
public interface EffectiveToolResolver {
     ...
     *
     * <p>{@code inheritRoleTools=false}（RESTRICT）时，Skill 工具声明只能收窄当前权限，不能扩张权限。
     * {@code inheritRoleTools=true}（INHERIT，仅限已审核系统 Skill）时，跳过
     * {@code skillRequiredToolNames} 限制，直接放行 Role 与 Agent 交集的全部业务工具。
     */
    ...
    /**
     * Assistant Skill 只能获得其必需工具与 Assistant、Agent 白名单的严格交集，并入
     * {@link BaseToolProfile}；{@code inheritRoleTools} 语义与 {@link #resolve} 一致。
     */
