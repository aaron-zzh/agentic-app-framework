 ...
 */
package com.xuejiai.aaf.framework.agentscope.gateway;
...
import com.xuejiai.aaf.framework.agentscope.session.PendingCompletion;
import com.xuejiai.aaf.framework.agentscope.session.SessionAgentManager;
import com.xuejiai.aaf.framework.agentscope.session.SessionConstants;
import com.xuejiai.aaf.framework.agentscope.session.SessionEntry;
import com.xuejiai.aaf.framework.agentscope.session.SessionFreshness;
import com.xuejiai.aaf.framework.agentscope.session.SessionFreshnessEvaluator;
import com.xuejiai.aaf.framework.agentscope.session.SessionKind;
import com.xuejiai.aaf.framework.agentscope.session.SessionResetPolicy;
import com.xuejiai.aaf.framework.agentscope.session.SessionView;
import com.xuejiai.aaf.framework.agentscope.session.SpawnResult;
 ...
 * analogue).
 *
 * <p>AgentStateStore management is delegated to {@link SessionAgentManager}. The gateway wires
 * itself as the {@link SessionAgentManager.AnnounceDispatcher} and {@link
...
public final class HarnessGateway implements Gateway {
    ...
    /**
     * session key -> last outbound address. Updated on every inbound {@link #run} call so proactive
     * outbound messages (announces) can be delivered to the correct channel/peer.
     */
    ...
    /**
     * Restores gateway routing maps from persisted MAIN sessions in the session store. Only fresh
     * sessions (per the configured {@link SessionResetPolicy}) are restored; stale sessions are
     * skipped so a new session will be created on the next inbound turn.
     */
    ...
    /**
     * Spawn interceptor: records gate-key, agent-id, and last-route mappings so announces for child
     * sessions route to the correct channel gate, agent, and outbound delivery target.
     */
