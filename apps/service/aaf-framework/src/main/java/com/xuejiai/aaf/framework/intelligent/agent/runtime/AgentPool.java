...
/**
 * Agent 池：按 AgentDefinition 池化复用 AgentExecutor 实例。
 * 借出前注入上下文，归还前重置内部状态（防止跨任务污染）。
 */
@Slf4j
@Component
...
public class AgentPool {
    ...
    public AgentExecutor borrow(AgentDefinition definition) {
        var queue = pool.computeIfAbsent(definition.getAgentId(), k -> new ConcurrentLinkedQueue<>());
