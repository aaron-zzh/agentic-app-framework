...
public final class AutomationApplicationService {
    ...
    public AutomationDefinition define(DefineCommand request) {
        var stored = delegatedTasks.find(request.tenantId(), request.sourceTaskId())
        ...
        var board = taskBoards.find(request.tenantId(), request.sourceTaskId())
        ...
        var template = new TaskTemplate(
                source.userId(), command.assistantId(), command.assistantVersion(), command.memorySubject(),
                command.input(), command.completionCriteria(), command.contextCandidates(), resetBoard(board, request.sourceTaskId()),
                source.contract(), window);
        var definition = new AutomationDefinition(
                request.tenantId(), request.automationId(), request.name(), version, request.sourceTaskId(),
                template, request.parameterSchema(), request.trigger(),
                new PermissionSnapshot(source.contract().allowedActions(), request.policyVersion(), now),
                request.failurePolicy(), request.notificationPlan(), AutomationDefinition.Lifecycle.DRAFT,
                false, false, false, now, now);
        var saved = definitions.save(definition);
        audit(saved, request.actorId(), "MODE_UPGRADE_TO_AUTOMATION", Map.of("sourceMode", "DELEGATED"));
        return saved;
    }

    public ImpactPreview preview(TenantId tenantId, String automationId, long version, String actorId) {
        var definition = requireVersion(tenantId, automationId, version);
        var policy = policies.get(tenantId);
        var prohibited = definition.permissionSnapshot().actions().stream()
                .filter(policy.extremeRiskActions()::contains).collect(java.util.stream.Collectors.toUnmodifiableSet());
        var preview = new ImpactPreview(prohibited.isEmpty(), definition.permissionSnapshot().actions(), prohibited,
                definition.template().contract().budget(), definition.trigger());
        definitions.save(definition.assessed(preview.allowed(), definition.dryRunPassed(), clock.instant()));
        audit(definition, actorId, "IMPACT_PREVIEW", Map.of("allowed", preview.allowed(), "prohibited", prohibited));
        return preview;
    }

    public DryRunResult dryRun(TenantId tenantId, String automationId, long version, Map<String, Object> parameters, String actorId) {
        var definition = requireVersion(tenantId, automationId, version);
        ...
        var result = new DryRunResult(passed, passed ? "dry-run 通过" : "必须先通过 impact preview", definition.template().board().subTasks().size());
    ...
    public AutomationDefinition publish(TenantId tenantId, String automationId, long version, String actorId) {
        var saved = definitions.save(requireVersion(tenantId, automationId, version).publish(clock.instant()));
    ...
    public AutomationDefinition disable(TenantId tenantId, String automationId, String actorId) {
        var saved = definitions.save(requireLatest(tenantId, automationId).disable(clock.instant()));
        audit(saved, actorId, "DISABLE", Map.of());
        return saved;
    }

    public AutomationRun trigger(TenantId tenantId, String automationId, String triggerKey,
            Map<String, Object> parameters, String actorId) {
        var definition = requireLatest(tenantId, automationId);
        ...
        var run = new AutomationRun(tenantId, runId, automationId, definition.version(), triggerKey,
                taskId, definition, parameters == null ? Map.of() : parameters,
                AutomationRun.Status.PENDING, null, now, now);
        var saved = runs.createOnce(run);
        audit(definition, actorId, "TRIGGER", Map.of("triggerKey", triggerKey, "runId", saved.runId()));
        return saved;
    }
    ...
    public OrganizationPolicy globalStop(TenantId tenantId, boolean stopped, String actorId) {
        ...
        var saved = policies.save(new OrganizationPolicy(tenantId, stopped, current.extremeRiskActions(),
                current.anomalyFailureThreshold(), current.policyVersion(), clock.instant()));
        audits.append(new AuditRecord(tenantId, UUID.randomUUID().toString(), "*", "GLOBAL_STOP_" + stopped,
                actorId, Map.of(), clock.instant()));
        return saved;
    }
    ...
    public int dispatchPending(int limit) {
        ...
        for (var run : runs.findPending(limit)) {
            try {
                policies.get(run.tenantId()).requireUnattended(run.definitionSnapshot().permissionSnapshot().actions());
                ...
                var failures = runs.history(run.tenantId(), run.automationId()).stream()
                        .filter(candidate -> candidate.status() == AutomationRun.Status.FAILED)
                        .count();
                if (failures >= policy.anomalyFailureThreshold()) {
                    var disabled = definitions.save(requireLatest(run.tenantId(), run.automationId()).disable(clock.instant()));
                    audit(disabled, "SYSTEM/anomaly-detector", "ANOMALY_AUTO_DISABLE",
                            Map.of("failureCount", failures, "threshold", policy.anomalyFailureThreshold()));
                }
            }
        }
        return dispatched;
    ...
    public List<AutomationDefinition> list(TenantId tenantId) { return definitions.list(tenantId); }
    public List<AuditRecord> auditHistory(TenantId tenantId, String automationId, Instant from, Instant to) {
    ...
    private AutomationDefinition requireLatest(TenantId tenantId, String automationId) {
        return definitions.findLatest(tenantId, automationId).orElseThrow(() -> new IllegalArgumentException("自动化不存在"));
    }
    private AutomationDefinition requireVersion(TenantId tenantId, String automationId, long version) {
        return definitions.findVersion(tenantId, automationId, version).orElseThrow(() -> new IllegalArgumentException("自动化版本不存在"));
    }
    private void audit(AutomationDefinition definition, String actorId, String action, Map<String, Object> details) {
        audits.append(new AuditRecord(definition.tenantId(), UUID.randomUUID().toString(), definition.automationId(),
                action, actorId, details, clock.instant()));
    }
    ...
    private static TaskBoard resetBoard(TaskBoard source, TaskId taskId) {
        ...
        source.subTasks().values().forEach(item -> reset.put(item.subTaskId(),
    ...
    public record DefineCommand(TenantId tenantId, String automationId, String name, TaskId sourceTaskId,
            ParameterSchema parameterSchema, TriggerSpec trigger, FailurePolicy failurePolicy,
            NotificationPlan notificationPlan, String policyVersion, String actorId) {}
    public record ImpactPreview(boolean allowed, Set<String> actions, Set<String> prohibitedActions,
            com.xuejiai.aaf.framework.intelligent.assistant.model.ExecutionContract.BudgetLimit budget,
            TriggerSpec trigger) {}
