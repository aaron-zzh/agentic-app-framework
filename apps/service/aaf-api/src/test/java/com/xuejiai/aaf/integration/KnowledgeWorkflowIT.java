...
/** 知识库→工作流跨模块集成测试 验证：创建知识库→导入文档→触发工作流 */
@SpringBootTest
...
class KnowledgeWorkflowIT {
    ...
    void should_import_document_to_knowledge_base() throws Exception {
        var file = new MockMultipartFile("file", "test.txt", "text/plain", "测试文档内容".getBytes());

        mockMvc.perform(
                        multipart("/api/knowledge/bases/{id}/documents", knowledgeBaseId)
                                .file(file))
                .andExpect(status().isOk());
    }
