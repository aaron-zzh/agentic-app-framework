...
import com.xuejiai.aaf.common.enums.billing.ResetCycleEnum;
...
public class EntitlementService implements EntitlementChecker {
    ...
    public void check(Long userId, String code, long cost) {
        var def = defRepository.findByCode(code)
        ...
        var quota = quotaRepository.findByUserIdAndEntId(userId, def.getId())
    ...
    public void instantiateQuotas(Long userId, Long planId) {
        ...
        for (var rule : rules) {
            var existing = quotaRepository.findByUserIdAndEntId(userId, rule.getEntId()).orElse(null);
            if (existing != null) {
                // 升级：更新额度
    ...
    public int resetExpiredQuotas() {
        ...
        for (var quota : expiredQuotas) {
            ...
            var subscription = subscriptionRepository.findByUserIdAndStatus(quota.getUserId(), "ACTIVE").orElse(null);
            if (subscription == null) continue;

            var rule = planEntitlementRepository.findByPlanIdAndEntId(subscription.getPlanId(), quota.getEntId())
                    .orElse(null);
            ...
            writeLedger(quota.getId(), rule.getQuota() - oldRemain, EntitlementOperationEnum.RESET, null, null);
        }
    ...
    private boolean canRefill(Long userId, EntitlementDef def, EntitlementQuota quota, long cost) {
        var subscription = subscriptionRepository.findByUserIdAndStatus(userId, "ACTIVE").orElse(null);
        if (subscription == null) return false;

        var rule = planEntitlementRepository.findByPlanIdAndEntId(subscription.getPlanId(), def.getId())
                .orElse(null);
    ...
    private boolean tryRefill(Long userId, EntitlementDef def, EntitlementQuota quota, long cost) {
        ...
        writeLedger(quota.getId(), refillAmount, EntitlementOperationEnum.REFILL, "ENT_REFILL", null);
        log.info("用户 {} 权益 {} 积分充值: 消耗积分={}, 充值额度={}", userId, def.getCode(), totalCreditCost, refillAmount);
        return true;
    }

    private void writeLedger(Long quotaId, long delta, EntitlementOperationEnum operation, String bizType, Long bizId) {
    ...
    private LocalDateTime calcNextReset(String resetCycle) {
        ...
        return switch (resetCycle) {
            ...
            case "YEARLY" -> now.plusYears(1).withMonth(1).withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0);
