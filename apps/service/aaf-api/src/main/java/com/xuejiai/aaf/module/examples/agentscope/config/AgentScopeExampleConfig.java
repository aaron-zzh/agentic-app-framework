 ...
 * <p>包含以下能力演示：
 * <ul>
 *   <li>① 基础聊天（basicChatAgent）</li>
 *   <li>⑧ RAG 知识库聊天（ragChatAgent）— 融入基础聊天，加 Knowledge 注入</li>
 *   <li>② 工具调用 + Hook/Tracing（toolCallingAgent）— 含 Langfuse Tracing 可选开关</li>
 *   <li>③ Supervisor 多智能体（supervisorAgent）</li>
 * </ul>
 */
@Slf4j
@Configuration
...
public class AgentScopeExampleConfig {
    ...
    /**
     * Langfuse Public Key（可选）。
     * 配置后自动启用 Langfuse Tracing，所有 Agent 调用链路上报到 Langfuse。
     ...
     *
     * <p>[Tracing能力点] AgentScope 通过 {@link TracerRegistry} 全局注册 Tracer，
     * 注册后所有 Agent 调用自动上报链路数据（无需修改 Agent 代码）。
     * 支持 OpenTelemetry 协议，可对接 Langfuse、Jaeger、Zipkin 等任意 OTLP 后端。
     */
    @PostConstruct
    ...
    public Model exampleDashScopeModel() {
        String key = StringUtils.hasText(dashScopeApiKey)
                ? dashScopeApiKey : System.getenv("AI_DASHSCOPE_API_KEY");
     ...
     * <ul>
     *   <li>{@link EmbeddingModel} — 将文本转为向量（此处用 DashScope text-embedding-v3）</li>
     *   <li>VectorStore — 存储和检索向量（此处用 InMemoryStore，生产可换 PgVector/Qdrant）</li>
     *   <li>{@link Knowledge} — 封装上述两者，提供 addDocuments/retrieve 接口</li>
     * </ul>
     */
     ...
     *
     * <p>[RAG能力点] {@code RAGMode.GENERIC}：每次 LLM 推理前自动检索知识库，
     * 将相关文档注入到 system prompt，无需 Agent 主动调用工具。
     * 对比 {@code RAGMode.AGENTIC}：Agent 通过 retrieve_knowledge 工具主动检索。
     */
    @Bean("ragChatAgent")
     ...
     *       PostReasoning→PreActing→PostActing→PostCall），演示链路追踪能力点</li>
     *   <li>{@link TokenMeteringHook} — AAF Token 计量 Hook，在 PostCallEvent 读取 getChatUsage()
     *       并记录用量，演示 Hook 与业务系统集成</li>
     * </ul>
     *
