 ...
 * Realtime TTS 示例配置。
 *
 * <p>演示 AgentScope {@link RealtimeTTSModel} 能力：流式文本转语音（TTS），
 * 支持增量推送文本，WebSocket 实时返回音频流，适合"边生成边播放"场景。
 *
 * <p>与普通 TTS 的区别：
 * <ul>
 *   <li>普通 TTS（TTSModel）：一次性输入完整文本，HTTP + SSE 返回音频</li>
 *   <li>Realtime TTS（RealtimeTTSModel）：流式输入 + WebSocket 流式输出，
 *       可在 LLM 生成文本的同时推送给 TTS，实现更低延迟的语音交互</li>
 * </ul>
 *
 * <p>需要外部服务：DashScope API（阿里云通义）
 * 模型：qwen3-tts-flash-realtime 或 cosyvoice-v2
 *
...
public class RealtimeExampleConfig {
     ...
     *
     * <p>[Realtime能力点] DashScopeRealtimeTTSModel 基于 WebSocket 传输，
     * 支持 push(text) 增量推送文本，finish() 获取剩余音频，
     * synthesizeStream(text) 一次性合成并流式返回。
     *
