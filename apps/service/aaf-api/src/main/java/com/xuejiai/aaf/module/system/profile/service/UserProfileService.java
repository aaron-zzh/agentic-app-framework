...
public class UserProfileService {
    ...
    public ProfileDimensionValue setValue(
        ...
        value.setConfidence("manual".equals(source) ? BigDecimal.ONE : new BigDecimal("0.80"));
        return valueRepository.save(value);
    }

    /** 构建 AI 上下文摘要——拼接 ai_visible=true 的维度值。 供 AssistantMessageHandler 注入 system prompt。 */
    public String buildAiContext(Long userId) {
        ...
        var dimNameMap =
                aiDimensions.stream().collect(Collectors.toMap(d -> d.getId(), d -> d.getName()));
        ...
        for (var v : values) {
            ...
            var display =
                    v.getValueText() != null
                            ? v.getValueText()
                            : v.getValueNumber() != null
                                    ? v.getValueNumber().toPlainString()
                                    : v.getValueTags();
