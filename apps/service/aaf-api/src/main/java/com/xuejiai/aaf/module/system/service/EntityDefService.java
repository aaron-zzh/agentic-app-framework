...
import com.xuejiai.aaf.module.system.repository.EntityDefRepository;
...
public class EntityDefService {
    ...
    public EntityDef getBySlug(String slug) {
        return entityDefRepository.findBySlug(slug)
                .orElseThrow(() -> new BusinessException(GlobalErrorCode.NOT_FOUND, "实体定义不存在: " + slug));
    }
    ...
    public CustomFieldVO addField(String slug, CustomFieldAddDTO dto) {
        ...
        // 检查字段名是否已存在
        var exists = fields.stream()
                .anyMatch(f -> dto.name().equals(f.get("name")));
        if (exists) {
    ...
    public void hideField(String slug, String fieldName) {
        ...
        var found = fields.stream()
                .filter(f -> fieldName.equals(f.get("name")))
                .findFirst()
                .orElseThrow(() -> new BusinessException(GlobalErrorCode.NOT_FOUND, "字段不存在: " + fieldName));
    ...
    private List<Map<String, Object>> getFieldsList(Map<String, Object> config) {
        ...
        if (fields instanceof List<?> list) {
            return new ArrayList<>(list.stream().map(item -> new HashMap<>((Map<String, Object>) item)).toList());
        }
    ...
    private EntityDefVO toVO(EntityDef e) {
        return new EntityDefVO(
                e.getId(), e.getSlug(), e.getConfig(),
                e.getBuiltin(), e.getEnabled(),
