 ...
 * 趋势统计服务。
 *
 * <p>核心指标：DAU/MAU、消息量、Token 消耗、收入。 使用原生 SQL + date_trunc 聚合，避免全量加载。
 */
@Slf4j
@Service
...
public class StatsService {
    ...
    public TrendSeriesVO queryTrend(TrendQueryDTO query) {
        ...
        return new TrendSeriesVO(
                categories, List.of(new TrendSeriesVO.Series(query.metric(), data)));
    }
    ...
    public TrendSeriesVO queryTrendWithComparison(TrendQueryDTO query) {
        ...
        return new TrendSeriesVO(
                categories,
                List.of(
                        new TrendSeriesVO.Series(
                                "当期", current.stream().map(TrendPointVO::value).toList()),
                        new TrendSeriesVO.Series(
                                "环比", previous.stream().map(TrendPointVO::value).toList())));
    }
    ...
    private void detectAnomaly(String metric, List<TrendPointVO> points) {
        ...
        if (prev > 0 && last > prev * ALERT_THRESHOLD) {
            monitorEngine.alert(
                    MonitorEngine.AlertLevel.WARNING,
                    "指标突增告警",
                    "指标 [%s] 突增：%d → %d（%.1f倍）"
                            .formatted(metric, prev, last, (double) last / prev));
        }
        if (prev > 0 && last < prev / ALERT_THRESHOLD) {
            monitorEngine.alert(
                    MonitorEngine.AlertLevel.WARNING,
