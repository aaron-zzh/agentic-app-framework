...
import com.xuejiai.aaf.framework.intelligent.ai.media.VideoGenerationService.TextToVideoRequest;
...
public class VideoGenerationTool {
    ...
    public record Request(String prompt, Integer durationSeconds, String resolution, String ratio) {}

    public record Response(String taskId, String status, String message) {}

    @Bean
    @Description("根据用户描述生成视频。参数：prompt(必填)-视频描述, durationSeconds(可选)-时长3~15秒, resolution(可选)-720P/1080P, ratio(可选)-16:9/9:16/1:1")
    public Function<Request, Response> generateVideo() {
        return request -> {
            log.info("对话触发视频生成: prompt={}", request.prompt());
            try {
                var videoRequest = new TextToVideoRequest(
                        request.prompt(), null, request.resolution(), request.ratio(),
