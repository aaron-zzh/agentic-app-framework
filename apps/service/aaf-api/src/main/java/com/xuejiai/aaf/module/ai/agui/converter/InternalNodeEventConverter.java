...
/**
 * 内部节点（非交付者）事件的投影：降级为 {@code CUSTOM}，payload 带 source 路径与原始事件类型；子任务生命周期事件
 * 额外追加标准 {@code StepStarted}/{@code StepFinished}（阶段通知）、{@code StateSnapshot}/{@code
 * StateDelta}（全局状态看板）与 {@code ActivitySnapshot}/{@code ActivityDelta}（对话时间线活动卡片，AAF-104
 * #10403）——四者都不替代 CUSTOM，是附加产出。
 *
 * <p><b>为什么标准事件是 CUSTOM 的兜底，不是替代</b>：AG-UI 的 {@code TEXT_MESSAGE_*} 语义是"面向用户的最终助手回复"。
 * 执行者、评估者的输出是中间产物，混入标准事件会让客户端把碎片当主回复渲染。官方 {@code SubagentEventConverter} 正是这样
 * 处理—— {@code source != null} 的事件一律 CUSTOM，且不按类型注册、只在有 source 时被调用。本类是同一策略在 AAF
 * 平级编排下的等价实现：AAF 的 source 由 {@link NodeIdentity#sourcePath()} 自行合成，而非依赖 core（core 在平级编排下
 * 恒为 null）。{@code StepStarted}/{@code StepFinished}/{@code StateSnapshot}/{@code StateDelta}/{@code
 * ActivitySnapshot}/{@code ActivityDelta} 是例外——它们不是"最终正文"，可以在 CUSTOM 之外额外发出标准事件供客户端
 * 统一处理，不需要客户端解析 CUSTOM payload。
 *
...
public final class InternalNodeEventConverter implements AafAguiEventConverter {
     ...
     *
     * <p>与 {@link #stateEvent} 同源、同触发时机，但服务不同 UI 位置——State 是全局共享文档，Activity 是嵌入
     * 对话消息时间线的独立卡片，见 {@code AafAguiStreamContext#subTaskActivityChanged} 的完整设计说明
     * （含已知的 assistant-ui 前端渲染缺口记录）。
     */
     ...
     *
     * <p>只在子任务生命周期终态/起始事件更新，与 {@link #stepEvent} 同源但语义不同——Step 是"事件流形式的阶段通知"，
     * State 是"结构化状态快照"，两者互补，分别服务 assistant-ui 的 {@code useAgUiState}（状态看板 UI）与
     * 事件订阅（进度提示）两类不同消费场景。
     */
     ...
     * 对同一组类型的处理是"per-run 状态操作"（{@code context.runStarted()}/{@code runFinishedOnce()}），
     * 若子任务的 {@code EXECUTION_STARTED} 也绕过降级直达该 converter，会把子任务的开始/结束误当整个 run 的开始/结束。
     * 因此阶段边界作为本 converter 的附加产出，而不是独立抢占类型分派权。
     *
     ...
     * AssistantCommand.nodeIdentityOf} 的交付角色算定，此时 {@code AGGREGATOR} 恰好是唯一交付者
     * （{@code delivery=true}），走 {@code RunLifecycleEventConverter} 的根节点路径而非本类——客户端视角是
     * "run 本身在收尾"（`RunStarted` 早已发出，聚合完成即 `RunFinished`），不需要独立 Step 包装。
     * {@code PASS_THROUGH}/{@code ORDERED_CONCAT} 两种契约则是纯拼接，没有独立智能体执行聚合工作。
     * 走到本类的子任务节点（{@code EXECUTOR}/{@code EVALUATOR} 恒内部，{@code COORDINATOR} 仅
     * {@code AGGREGATOR_REDUCE} 下为内部）统一映射为 {@code "execution"}。
     */
    private static List<AguiEvent> stepEvent(ExecutionEventType type, AafAguiStreamContext context) {
        return switch (type) {
