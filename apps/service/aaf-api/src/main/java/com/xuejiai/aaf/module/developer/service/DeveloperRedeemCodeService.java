...
public class DeveloperRedeemCodeService {
        ...
        if ("LICENSE".equals(dto.type())) {
            ...
            if (planCode == null || planCode.isBlank()) {
                throw new BusinessException(
                        GlobalErrorCode.BAD_REQUEST, "LICENSE 类型兑换码必须指定套餐 planCode");
            }
            var plan =
                    planRepository
                            .findByCode(planCode)
            ...
            if (dto.tokenAmount() == null || dto.tokenAmount() < 1) {
                throw new BusinessException(
                        GlobalErrorCode.BAD_REQUEST, "TOKEN 类型兑换码必须指定 tokenAmount");
            }
    ...
    public Object redeem(Long developerId, String rawCode) {
        ...
        if ("LICENSE".equals(code.getType())) {
            ...
            // 发放 token
            tokenService.earn(
                    developerId, code.getTokenAmount(), "REDEEM_CODE", code.getCodePrefix());
            return code.getTokenAmount();
        }
    }

    /** 公开激活接口专用：无需登录，验证 LICENSE 类型兑换码，返回预签发的 license.jwt。 兑换码标记为 REDEEMED，一次性有效。 */
    @Transactional
