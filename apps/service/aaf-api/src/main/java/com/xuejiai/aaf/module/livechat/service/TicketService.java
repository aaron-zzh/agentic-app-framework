...
public class TicketService {
    ...
    public Ticket create(TicketCreateDTO dto) {
        ...
        addRecord(saved, TicketOperationEnum.CREATE, dto.userId(), null, TicketStatusEnum.PENDING, null);
        return saved;
    }
    ...
    public void assign(Long ticketId, Long assigneeId, Long operatorId) {
        ...
        addRecord(ticket, TicketOperationEnum.ASSIGN, operatorId, fromStatus, ticket.getStatus(), null);
    }
    ...
    public void submitConfirm(Long ticketId, Long operatorId, String remark) {
        ...
        ticketRepository.save(ticket);
        addRecord(ticket, TicketOperationEnum.CONFIRM, operatorId, fromStatus, ticket.getStatus(), remark);
    }
    ...
    public void close(Long ticketId, Long operatorId, String remark) {
        ...
        ticket.close();
        ticketRepository.save(ticket);
        addRecord(ticket, TicketOperationEnum.CLOSE, operatorId, fromStatus, ticket.getStatus(), remark);
    }
    ...
    public void reopen(Long ticketId, Long operatorId, String remark) {
        ...
        ticket.reopen();
        ticketRepository.save(ticket);
        addRecord(ticket, TicketOperationEnum.REOPEN, operatorId, fromStatus, ticket.getStatus(), remark);
    }
    ...
    public void transfer(Long ticketId, Long newAssigneeId, Long operatorId, String remark) {
        ...
        ticketRepository.save(ticket);
        addRecord(ticket, TicketOperationEnum.TRANSFER, operatorId, fromStatus, ticket.getStatus(), remark);
    }
    ...
    public List<Ticket> listByAssignee(Long assigneeId) {
        return ticketRepository.findByAssigneeIdAndStatusIn(assigneeId,
                List.of(TicketStatusEnum.PENDING, TicketStatusEnum.PROCESSING, TicketStatusEnum.CONFIRMING));
    }
    ...
    public List<Ticket> scanOverdue() {
        var overdueTickets = ticketRepository.findByStatusInAndSlaDueTimeBefore(
                List.of(TicketStatusEnum.PENDING, TicketStatusEnum.PROCESSING, TicketStatusEnum.CONFIRMING),
                LocalDateTime.now());
        overdueTickets.forEach(t ->
                log.warn("工单 SLA 超时: ticketNo={}, priority={}, slaDue={}", t.getTicketNo(), t.getPriority(), t.getSlaDueTime()));
    ...
    public TicketStatVO getStatistics(LocalDateTime since) {
        ...
        // 按状态统计
        Map<String, Long> statusDist = ticketRepository.countByStatus().stream()
                .collect(Collectors.toMap(r -> ((TicketStatusEnum) r[0]).getCode(), r -> (Long) r[1]));
        // 按类型统计
        Map<String, Long> typeDist = ticketRepository.countByType().stream()
                .collect(Collectors.toMap(r -> ((TicketTypeEnum) r[0]).getCode(), r -> (Long) r[1]));
        return new TicketStatVO(total, closed, overdue, resolveRate, overdueRate, statusDist, typeDist);
    }
    ...
    private void addRecord(Ticket ticket, TicketOperationEnum op, Long operatorId,
                           TicketStatusEnum from, TicketStatusEnum to, String remark) {
