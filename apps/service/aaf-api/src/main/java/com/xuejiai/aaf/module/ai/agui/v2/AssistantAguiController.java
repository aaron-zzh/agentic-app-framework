...
public class AssistantAguiController {
    ...
    public AssistantAguiController(AssistantCommandPort assistants, OperatorContext operators) {
        this.assistants = assistants; this.operators = operators;
    }
    ...
    public SseEmitter run(@RequestBody RunRequest request) {
        var tenantId = tenant();
        var userId = new UserId(operators.currentOwnerId().map(String::valueOf)
                .orElseThrow(() -> new AccessDeniedException("请求未认证")));
        var now = Instant.now();
        var command = new AssistantCommand(AssistantCommand.Operation.START, tenantId, userId,
                new MemorySubject(tenantId, SubjectKind.USER, userId.value()),
                new AssistantId(request.assistantId()), new AssistantVersion(request.assistantVersion()),
                new ConversationId(request.threadId()), new SessionId(request.threadId()),
                new TaskId(request.taskId()), new ExecutionId(request.runId()), new RunId(request.runId()),
                null, new CorrelationId(request.taskId()), null,
                new IdempotencyKey(request.idempotencyKey()), request.controlMode(), request.executionContract(),
                null, 0, request.input(), CompletionCriteria.responseDelivered(), List.of(), now);
        var emitter = new SseEmitter(600_000L);
        assistants.execute(command).subscribe(event -> send(emitter, event), emitter::completeWithError, emitter::complete);
        return emitter;
    }

    private static void send(SseEmitter emitter, ExecutionEvent event) {
        try {
            emitter.send(SseEmitter.event().id(Long.toString(event.sequence())).name(event.type().name())
    ...
    public record RunRequest(String assistantId, long assistantVersion, String threadId, String taskId,
            String runId, String idempotencyKey, ControlMode controlMode, ExecutionContract executionContract,
            String input) {}
}
