 ...
 * <p>PlanNotebook 工作原理：
 * <ol>
 *   <li>Agent 收到复杂任务后，调用 {@code create_plan} 工具创建计划（含子任务列表）</li>
 *   <li>每次 LLM 推理前，PlanNotebook 自动注入 {@code <system-hint>} 提示当前进度</li>
 *   <li>Agent 按子任务顺序执行，调用 {@code finish_subtask} 标记完成</li>
 *   <li>所有子任务完成后调用 {@code finish_plan} 结束计划</li>
 * </ol>
 *
...
public class PlanExampleConfig {
     ...
     *
     * <p>[Plan能力点] {@code .enablePlan()} 等价于注册默认 PlanNotebook，
     * 自动向 Toolkit 注册 10 个 Plan 工具函数，并注册 Hook 在每次推理前注入进度提示。
     * 也可用 {@code .planNotebook(PlanNotebook.builder()...build())} 自定义配置。
     */
    @Bean("planAgent")
