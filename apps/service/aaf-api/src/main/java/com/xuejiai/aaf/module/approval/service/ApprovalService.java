...
public class ApprovalService {
    ...
    public void handleTimeout(ApprovalNodeConfig config, String taskId) {
        ...
        switch (strategy) {
            case AUTO_APPROVE ->
                    bpmnEngine.completeTask(taskId, Map.of("approved", true), "审批超时自动通过");
            case AUTO_REJECT ->
                    bpmnEngine.completeTask(taskId, Map.of("approved", false), "审批超时自动拒绝");
            case TRANSFER -> {
                var target =
                ...
                approvalOperationService.transferSign(taskId, target, "审批超时自动转交");
            }
            case REMIND -> approvalOperationService.urgeTask(taskId, "system-timeout");
        }
    }
