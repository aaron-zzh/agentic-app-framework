...
/**
 * 把 {@code CLARIFICATION_REQUESTED} 事件 payload 里的 {@code questions} 转换为标准 JSON Schema，
 * 供 {@link AguiEvent.Interrupt#responseSchema()} 使用（AAF-114 #11408 第二版）。
 *
 * <p>输入是 {@code DelegatedTaskCoordinator.clarificationRequestedEvent} 构造的
 * {@code List<Map<String,Object>>}，每项含 {@code field}/{@code question}/{@code options}。
 * 首批只覆盖 text 和单选枚举两种（对齐 {@code ClarificationRequest.values: Map<String,String>} 的现有能力，
 * 不超前设计模型尚不支持的 number/checkbox/multiple——那些需要先扩展 {@code ClarificationRequest.Question}
 * 领域模型才有意义，留作后续迭代）。
 */
final class ClarificationResponseSchema {
    ...
    /**
     * @param questions 每项含 {@code field}(String)/{@code question}(String)/{@code options}(List&lt;String&gt;)
     * @param requiredFields 必填字段名列表
     * @return 标准 JSON Schema：{@code {"type":"object","properties":{...},"required":[...]}}
     */
    @SuppressWarnings("unchecked")
    static Map<String, Object> fromQuestions(
            List<?> questions, List<String> requiredFields) {
