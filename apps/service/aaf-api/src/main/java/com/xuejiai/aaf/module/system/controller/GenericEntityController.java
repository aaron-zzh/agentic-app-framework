...
public class GenericEntityController {
    ...
    public Result<List<Map<String, Object>>> list(@PathVariable String slug) {
        ...
        var sql = "SELECT * FROM %s WHERE deleted = false ORDER BY create_time DESC".formatted(tableName);
    ...
    @PostMapping
    public Result<Map<String, Object>> create(@PathVariable String slug, @RequestBody Map<String, Object> body) {
        var tableName = resolveTable(slug);
        if (body.isEmpty()) {
        ...
        var sql = "INSERT INTO %s (%s) VALUES (%s) RETURNING *".formatted(
    ...
    public Result<Map<String, Object>> update(
            @PathVariable String slug, @PathVariable Long id, @RequestBody Map<String, Object> body) {
        var tableName = resolveTable(slug);
        if (body.isEmpty()) {
            throw new BusinessException(GlobalErrorCode.BAD_REQUEST, "请求体不能为空");
        }
        ...
        values.add(id);
        var sql = "UPDATE %s SET %s, update_time = CURRENT_TIMESTAMP WHERE id = ? AND deleted = false RETURNING *"
    ...
    public Result<Void> delete(@PathVariable String slug, @PathVariable Long id) {
        var tableName = resolveTable(slug);
        var sql = "UPDATE %s SET deleted = true WHERE id = ? AND deleted = false".formatted(tableName);
