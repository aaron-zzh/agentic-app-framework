 ...
 *
 * <p>业务代码（注册、邀请、探索奖励等）只需调用 grant(userId, ruleCode, bizId)， 不需要关心积分数量和有效期，由规则表统一配置。
 */
@Slf4j
@Service
...
public class CreditGrantService {
    ...
    public long grant(Long userId, String code, String bizId) {
        ...
        creditService.earnBatch(userId, rule.getAmount(), code, code, bizId, rule.calcExpireAt());
