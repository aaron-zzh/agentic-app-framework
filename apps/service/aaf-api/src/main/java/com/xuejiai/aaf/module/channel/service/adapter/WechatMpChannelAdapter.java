 ...
 * 微信公众号渠道适配器。
 *
 * <p>消息接收（文本/图片/语音/事件）与被动回复、模板消息推送、客服消息。
 * 需配置 aaf.channel.wx.mp.enabled=true 激活。
 */
@Slf4j
@Component
...
public class WechatMpChannelAdapter implements ChannelAdapter {
    ...
    public UnifiedMessage receive(String rawPayload) {
        ...
        return switch (msgType) {
            ...
            case "image" -> new UnifiedMessage(
                    ...
                    null,
                    xmlMsg.getPicUrl(),
                    null, null, Map.of("mediaId", xmlMsg.getMediaId()),
                    rawPayload,
                    java.time.LocalDateTime.now());
            case "voice" -> new UnifiedMessage(
                    ChannelTypeEnum.WECHAT_MP,
                    com.xuejiai.aaf.common.enums.channel.MessageDirectionEnum.INBOUND,
    ...
    public void reply(UnifiedMessage message) {
        try {
            var kefuMsg = switch (message.messageType()) {
                case TEXT -> WxMpKefuMessage.TEXT()
                        .toUser(message.externalUserId())
                        .content(message.content())
                        .build();
                case IMAGE -> WxMpKefuMessage.IMAGE()
                        .toUser(message.externalUserId())
                        .mediaId((String) message.extra().get("mediaId"))
                        .build();
                default -> WxMpKefuMessage.TEXT()
                        .toUser(message.externalUserId())
            ...
            log.error("微信公众号回复失败: user={}, error={}",
    ...
    public void pushTemplate(
        ...
        try {
            var templateMsg = WxMpTemplateMessage.builder()
                    .toUser(externalUserId)
                    .templateId(templateId)
                    .build();
            variables.forEach(
                    (k, v) -> templateMsg.addData(new WxMpTemplateData(k, v)));
            wxMpService.getTemplateMsgService().sendTemplateMsg(templateMsg);
        } catch (Exception e) {
            log.error("微信模板消息推送失败: user={}, template={}, error={}",
                    externalUserId, templateId, e.getMessage());
        }
    }
}
