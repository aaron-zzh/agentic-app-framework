...
public class DefinitionLifecycleController {
    ...
    public DefinitionLifecycleController(
            DefinitionLifecycleService service, OperatorContext operators) {
        this.service = service;
        this.operators = operators;
    }

    @PostMapping("/{kind}/{id}/versions/{version}/draft")
    public Result<DefinitionLifecycle> draft(
            @PathVariable DefinitionKind kind,
    ...
    public Result<DefinitionLifecycle> review(
            @PathVariable DefinitionKind kind,
            @PathVariable String id,
            @PathVariable long version,
            @RequestBody ReviewDTO dto) {
        return Result.success(
                service.review(tenant(), kind, id, version, dto.status(), actor(), dto.reason()));
    }

    @PostMapping("/{kind}/{id}/versions/{version}/publish")
    public Result<DefinitionLifecycle> publish(
            @PathVariable DefinitionKind kind,
            @PathVariable String id,
            @PathVariable long version) {
        return Result.success(service.publish(tenant(), kind, id, version));
    }

    @PostMapping("/{kind}/{id}/versions/{version}/deprecate")
    public Result<DefinitionLifecycle> deprecate(
            @PathVariable DefinitionKind kind,
            @PathVariable String id,
            @PathVariable long version) {
        return Result.success(service.deprecate(tenant(), kind, id, version));
    }

    @PostMapping("/{kind}/{id}/versions/{version}/disable")
    public Result<DefinitionLifecycle> disable(
            @PathVariable DefinitionKind kind,
            @PathVariable String id,
            @PathVariable long version) {
        return Result.success(service.disable(tenant(), kind, id, version));
    }

    @PostMapping("/{kind}/{id}/versions/{version}/rollback/{targetVersion}")
    public Result<DefinitionLifecycle> rollback(
            @PathVariable DefinitionKind kind,
            @PathVariable String id,
            @PathVariable long version,
            @PathVariable long targetVersion) {
        return Result.success(service.rollback(tenant(), kind, id, version, targetVersion));
    }
