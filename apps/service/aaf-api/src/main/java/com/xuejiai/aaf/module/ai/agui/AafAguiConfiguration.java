...
/**
 * AG-UI 链路配置——用 {@link AafAgentResolver} 替换默认 resolver， 覆盖 starter 的 {@link AguiRestController}，使
 * /agui/runs 链路具备 上下文设置 + 历史播种 + 记忆检索注入 + Agent 状态持久化能力。
 */
@Configuration
public class AafAguiConfiguration {
    ...
    public AguiRequestProcessor aafAguiRequestProcessor(
        ...
        var resolver =
                new AafAgentResolver(
                        sessionManager, chatSessionResolver, assistantRuntime, agentScopeSession);
        var config =
                AguiAdapterConfig.builder()
                        ...
                        .build();
        return AguiRequestProcessor.builder().agentResolver(resolver).config(config).build();
    }
