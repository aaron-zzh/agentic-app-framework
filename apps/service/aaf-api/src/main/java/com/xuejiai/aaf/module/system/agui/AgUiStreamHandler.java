    ...
    public void handleStream(Flux<ChatResponse> flux, SseEmitter emitter, String runId) {
        ...
        flux.doOnNext(response -> {
            ...
            if (text != null && !text.isEmpty()) {
                sendEvent(emitter, AgUiEvent.textMessageContent(runId, messageId, text));
            }
            ...
            if (toolCalls != null && !toolCalls.isEmpty()) {
                for (var toolCall : toolCalls) {
                    var toolCallId = toolCall.id() != null ? toolCall.id() : UUID.randomUUID().toString();
                    sendEvent(emitter, AgUiEvent.toolCallStart(runId, toolCallId, toolCall.name()));
                    if (toolCall.arguments() != null) {
                        sendEvent(emitter, AgUiEvent.toolCallArgs(runId, toolCallId, toolCall.arguments()));
                    }
