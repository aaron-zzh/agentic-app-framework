...
public class OcrTool {
    ...
    private final ObjectProvider<OcrService> ocrService;
    ...
    public String recognizeOcr(@ToolParam(description = "OCR 识别 JSON 参数") String requestJson) {
        try {
            var service = ocrService.getIfAvailable();
            if (service == null) {
                return asJson(ToolCallResult.error(TOOL_NAME, "TOOL_UNAVAILABLE", "OCR 服务未启用"));
            }
