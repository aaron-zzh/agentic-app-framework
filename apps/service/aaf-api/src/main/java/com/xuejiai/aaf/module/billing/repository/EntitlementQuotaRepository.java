...
public interface EntitlementQuotaRepository extends CrudEntityRepository<EntitlementQuota> {
     ...
     *
     * <p>{@code check}（只读预判）与 {@code consume}（真扣）分处两个事务，若扣减仍走无锁的读改写， 并发请求会同时读到相同 remain
     * 后各自写回，导致丢失更新甚至 remain 变负。与积分侧 {@code CreditAccountRepository#findByUserIdForUpdate} 保持同一并发策略。
     */
