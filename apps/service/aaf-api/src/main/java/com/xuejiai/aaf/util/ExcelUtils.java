...
public final class ExcelUtils {
    ...
    private static void setResponseHeaders(
            HttpServletResponse response, String filename, ExcelTypeEnum type) {
        String suffix = type == ExcelTypeEnum.CSV ? ".csv" : ".xlsx";
        String contentType =
                type == ExcelTypeEnum.CSV
    ...
    public static <T> List<T> read(MultipartFile file, Class<T> head) throws IOException {
        try (InputStream in = file.getInputStream()) {
            return FesodSheet.read(in, head, null).autoCloseStream(false).doReadAllSync();
        }
    }
}
