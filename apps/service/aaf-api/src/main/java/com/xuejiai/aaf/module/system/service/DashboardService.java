...
import com.xuejiai.aaf.module.system.domain.DashboardWidget;
import com.xuejiai.aaf.module.system.repository.DashboardRepository;
import com.xuejiai.aaf.module.system.repository.DashboardWidgetRepository;
import com.xuejiai.aaf.module.system.vo.DashboardCreateDTO;
import com.xuejiai.aaf.module.system.vo.DashboardUpdateDTO;
import com.xuejiai.aaf.module.system.vo.DashboardVO;
import com.xuejiai.aaf.module.system.vo.DashboardWidgetVO;
import com.xuejiai.aaf.module.system.vo.WidgetCreateDTO;
import com.xuejiai.aaf.module.system.vo.WidgetDataVO;
...
public class DashboardService {
    ...
    public DashboardVO getById(Long id) {
        var dashboard = dashboardRepository.findById(id)
                .orElseThrow(() -> new BusinessException(GlobalErrorCode.NOT_FOUND, "仪表盘不存在"));
    ...
    public WidgetDataVO getWidgetData(Long widgetId) {
        var widget = widgetRepository.findById(widgetId)
                .orElseThrow(() -> new BusinessException(GlobalErrorCode.NOT_FOUND, "组件不存在"));
    ...
    private Object queryCounter(Map<String, Object> config) {
        ...
        var sql = "count".equals(aggregation)
    ...
    private Object queryChart(Map<String, Object> config) {
        ...
        var sql = "SELECT %s AS label, COUNT(%s) AS value FROM %s WHERE deleted = false GROUP BY %s ORDER BY %s"
    ...
    private Object queryList(Map<String, Object> config) {
        ...
        var cols = columns.stream().map(this::sanitizeIdentifier).reduce((a, b) -> a + ", " + b).orElse("*");
        var sql = "SELECT %s FROM %s WHERE deleted = false ORDER BY create_time DESC LIMIT %d"
    ...
    private DashboardVO toVO(Dashboard d) {
        var widgets = widgetRepository.findByDashboardIdOrderBySortOrder(d.getId()).stream()
                .map(w -> new DashboardWidgetVO(
                        w.getId(), w.getType(), w.getTitle(), w.getPosition(), w.getConfig(), w.getSortOrder()))
                .toList();
        return new DashboardVO(
                d.getId(), d.getName(), d.getDescription(), d.getIsDefault(),
                widgets, d.getCreateTime(), d.getUpdateTime());
    }
}
