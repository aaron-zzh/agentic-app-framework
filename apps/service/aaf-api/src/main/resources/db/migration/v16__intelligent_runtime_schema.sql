...
CREATE TABLE ai_execution_profile_snapshot (
    ...
    CONSTRAINT uk_execution_profile_tenant_execution UNIQUE (tenant_id, execution_id),
    CONSTRAINT ck_execution_profile_assistant_revision CHECK (assistant_revision >= 0)
);

COMMENT ON TABLE ai_execution_profile_snapshot IS '单次执行不可变画像：冻结 Assistant/Role/Skill/Tool/Model，供审计与任务恢复';
...
COMMENT ON COLUMN ai_execution_profile_snapshot.execution_id IS '执行稳定标识，同一执行只能冻结一次';
...
COMMENT ON COLUMN ai_execution_profile_snapshot.frozen_at IS '画像冻结时间';
