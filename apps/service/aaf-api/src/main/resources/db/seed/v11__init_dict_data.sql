...
-- sys_file_storage 文件存储类型（与 framework StorageType 枚举一致）
INSERT INTO sys_dict_data (dict_type, label, value, sort, color_type) VALUES
('sys_file_storage', '本地存储', 'LOCAL', 1, 'default'),
('sys_file_storage', 'S3 兼容（MinIO / AWS S3）', 'S3', 2, 'primary'),
('sys_file_storage', '阿里云 OSS', 'OSS', 3, 'success')
ON CONFLICT DO NOTHING;
...
INSERT INTO sys_dict_type (name, type, status, remark) VALUES
('佣金流水状态', 'brokerage_record_status',   0, 'FROZEN=冻结中 / VALID=可用 / CANCELLED=已取消'),
('佣金业务类型', 'brokerage_record_biz_type', 0, 'ORDER/SUBSCRIBE/RECHARGE/INVITE/ORDER_REFUND'),
('提现类型',     'brokerage_withdraw_type',   0, 'WECHAT/ALIPAY/BANK'),
('提现状态',     'brokerage_withdraw_status', 0, 'PENDING/APPROVED/REJECTED/TRANSFERRED')
ON CONFLICT DO NOTHING;

-- brokerage_record_status 佣金流水状态
INSERT INTO sys_dict_data (dict_type, label, value, sort, color_type) VALUES
('brokerage_record_status', '冻结中', 'FROZEN',    1, 'warning'),
('brokerage_record_status', '可用',   'VALID',     2, 'success'),
('brokerage_record_status', '已取消', 'CANCELLED', 3, 'info')
ON CONFLICT DO NOTHING;

-- brokerage_record_biz_type 佣金业务类型
INSERT INTO sys_dict_data (dict_type, label, value, sort, color_type) VALUES
('brokerage_record_biz_type', '商品订单', 'ORDER',        1, 'primary'),
('brokerage_record_biz_type', '会员订阅', 'SUBSCRIBE',    2, 'success'),
('brokerage_record_biz_type', '积分充值', 'RECHARGE',     3, 'info'),
('brokerage_record_biz_type', '邀请注册', 'INVITE',       4, 'warning'),
('brokerage_record_biz_type', '退款冲回', 'ORDER_REFUND', 5, 'danger')
ON CONFLICT DO NOTHING;

-- brokerage_withdraw_type 提现类型
INSERT INTO sys_dict_data (dict_type, label, value, sort, color_type) VALUES
('brokerage_withdraw_type', '微信',   'WECHAT', 1, 'success'),
('brokerage_withdraw_type', '支付宝', 'ALIPAY', 2, 'primary'),
('brokerage_withdraw_type', '银行卡', 'BANK',   3, 'default')
ON CONFLICT DO NOTHING;

-- brokerage_withdraw_status 提现状态
INSERT INTO sys_dict_data (dict_type, label, value, sort, color_type) VALUES
('brokerage_withdraw_status', '待审核', 'PENDING',     1, 'warning'),
('brokerage_withdraw_status', '审核通过', 'APPROVED',  2, 'primary'),
