...
export const systemConfigApi = {
  ...
  listByCategory: (category: string) =>
    backendApi.get<SystemConfigVO[]>(`/system/configs?category=${category}`),

  /** 查全部配置 */
  listAll: () => backendApi.get<SystemConfigVO[]>(`/system/configs?category=*`),

  /** 更新配置值 */
  update: (key: string, value: string) => backendApi.put<void>(`/system/configs`, { key, value }),
  ...
  getPublic: (key: string) =>
    backendApi.get<string>(`/public/system/configs/${encodeURIComponent(key)}`)
}
