...
function formatCredits(m: PublicModelPricingVO): string {
  switch (m.quotaType) {
    case 1:
      return m.creditPerUse != null ? `${m.creditPerUse} 积分/次` : "—"
    case 2:
      return m.creditPerSec != null ? `${m.creditPerSec} 积分/秒` : "—"
    case 3:
      return m.creditPerUnit != null ? `${m.creditPerUnit} 积分/张` : "按规格"
    default:
    ...
    <article className="flex flex-col gap-3 rounded-xl border bg-card p-4 transition-all hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="flex flex-wrap gap-1">
        {caps.slice(0, 3).map((c) => (
          <span
            key={c}
            className="rounded-md bg-foreground/[0.05] px-1.5 py-0.5 text-[10px] text-muted-foreground"
          >
            {CAP_LABEL[c] ?? c}
          </span>
        ))}
...
export default function StudioMePricingPage() {
  ...
  const kindCounts = useMemo(() => {
    ...
    for (const m of models)
      counts.set(toKind(m.capabilities ?? ""), (counts.get(toKind(m.capabilities ?? "")) ?? 0) + 1)
    return [
  ...
  return (
    <div className="flex h-full">
      {/* 左侧类型筛选 */}
      <aside className="w-40 shrink-0 space-y-1 overflow-y-auto border-r p-4">
