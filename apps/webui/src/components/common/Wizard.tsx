    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg rounded-lg border bg-background shadow-xl">
        ...
        <div className="border-b px-6 py-4">
          <h2 className="font-semibold text-lg">{title ?? "向导"}</h2>
        </div>
        ...
        <div className="flex items-center gap-2 border-b px-6 py-3">
          {steps.map((s, i) => (
            <div key={s.label} className="flex items-center gap-2">
              <span
                className={`flex h-6 w-6 items-center justify-center rounded-full text-xs ${i <= currentStep ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {i + 1}
              </span>
              <span
        ...
        {/* 内容 */}
        <FormProvider {...form}>
          <div className="min-h-[200px] px-6 py-4">{step?.content}</div>
        </FormProvider>
