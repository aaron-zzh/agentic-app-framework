...
export function ObjectDetailPanel({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: 切换对象时必须重置面板本地状态
  useEffect(() => {
    setCompareIds([])
    ...
    <Sheet open={open && object !== undefined} onOpenChange={onOpenChange}>
      <SheetContent className="w-[96vw] sm:max-w-3xl">
        <SheetHeader>
          ...
          <SheetDescription>
            {object
              ? `${OBJECT_TYPE_LABELS[object.objectType]} · #${object.stableKey}${lifecycleWritable ? "" : ` · ${project.status} 阶段只读`}`
              : "查看项目对象"}
          </SheetDescription>
        </SheetHeader>
        ...
        {object ? (
          <Tabs defaultValue="detail" className="min-h-0 flex-1 px-4 pb-4">
            <TabsList className="w-full">
              ...
              {object.objectType === "deliverable_set" ? (
            ...
            <TabsContent value="detail" className="min-h-0">
              <ScrollArea className="h-full pr-3">
                <dl className="grid grid-cols-[7rem_minmax(0,1fr)] gap-x-3 gap-y-3 rounded-xl border p-4 text-sm">
                  ...
                  <dd>{object.id}</dd>
                  <dt className="text-muted-foreground">稳定 Key</dt>
                  <dd>{object.stableKey}</dd>
                  <dt className="text-muted-foreground">模板实例</dt>
                  <dd>
                    {object.blueprintTemplateKey
                      ? `${object.blueprintTemplateKey} / ${object.instanceNo ?? "-"}`
                      : "自定义对象"}
                  </dd>
                  <dt className="text-muted-foreground">合同角色</dt>
                  <dd>{object.contractRole ?? "-"}</dd>
                ...
                <pre className="mt-4 max-h-80 overflow-auto rounded-xl border bg-muted/40 p-3 text-xs">
                ...
                {versionsQuery.isLoading ? (
                  ...
                  <Empty className="min-h-56">
                    <EmptyHeader>
                      ...
                      <EmptyDescription>执行成功后会先产生不可变候选版本。</EmptyDescription>
                    </EmptyHeader>
                  </Empty>
                ) : (
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center justify-between gap-3 rounded-xl border bg-muted/20 p-3">
                      <p className="text-muted-foreground text-sm">
                        勾选任意两个候选或采用版进行比较（{compareIds.length}/2）
                      </p>
                      <Button
                        type="button"
                        variant="outline"
                    ...
                    {versions.map((version) => (
                      <div key={version.id} className="flex flex-col gap-2 rounded-xl border p-3">
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            <Checkbox
                              ...
                              aria-label={`选择版本 ${version.versionNo} 比较`}
                              onCheckedChange={(checked) =>
                                toggleCompare(version.id, checked === true)
                              }
                            />
                          ...
                          {version.status === "candidate" && canAdopt ? (
                            <div className="flex gap-2">
                              <Button
                                ...
                                variant="outline"
                                size="xs"
                                disabled={!canAdoptVersion || reject.isPending}
                                onClick={() => {
                                  if (!canAdoptVersion) return
                                  setReason("")
                              ...
                              <Button
                                type="button"
                                size="xs"
                                disabled={!canAdoptVersion || adopt.isPending}
                                onClick={() => {
                                  if (!canAdoptVersion) return
                                  setReason("")
                                  setAdoptId(version.id)
                                }}
                              >
                                {object.adoptedVersionId ? "替换采用" : "采用"}
                              </Button>
                            </div>
                          ) : null}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-x-4 text-muted-foreground text-xs">
                          <span>Run #{version.executionRunId ?? "-"}</span>
                ...
                {runsQuery.isLoading ? (
                  ...
                  <Empty className="min-h-56">
                    <EmptyHeader>
                      ...
                      <EmptyDescription>该对象还没有关联 ExecutionRun。</EmptyDescription>
                    </EmptyHeader>
                  </Empty>
                ) : (
                  <div className="flex flex-col gap-3">
                    <ExecutionRunTree roots={runTrees} canAction={canRunAction} />
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground text-xs">
                        Root {Math.min((runPage - 1) * 20 + 1, runTotal)}–
                        {Math.min(runPage * 20, runTotal)} / {runTotal}
                      </span>
                      <div className="flex gap-2">
                        <Button
                          type="button"
                        ...
                        <Button
                          ...
                          size="xs"
                          variant="outline"
                          disabled={runPage * 20 >= runTotal || runsQuery.isFetching}
            ...
            {timelineAvailable ? (
              <TabsContent value="timeline" className="min-h-0">
                <TimelinePanel
                  project={project}
                  object={object}
                  lifecycleWritable={lifecycleWritable}
                  canCreate={canTimelineCreate}
                  canUpdate={canTimelineUpdate}
                  canDelete={canTimelineDelete}
                />
              </TabsContent>
            ) : null}
          </Tabs>
      ...
      {object ? (
        <CandidateComparisonDialog
          ...
          projectId={project.id}
          objectId={object.id}
          leftVersionId={compareIds[0] ?? null}
          rightVersionId={compareIds[1] ?? null}
          onOpenChange={setCompareOpen}
        />
      ) : null}

      <Dialog
        open={adoptId !== null && canAdoptVersion}
        onOpenChange={(next) => {
          if (!next) {
            setAdoptId(null)
            setReason("")
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{replacing ? "确认替换采用版本" : "采用候选版本"}</DialogTitle>
            <DialogDescription>
              {replacing
                ? `当前采用版 #${object?.adoptedVersionId} 将被版本 ${selectedAdopt?.versionNo ?? ""} 替换；CAS 冲突会刷新权威状态。`
                : "采用只移动当前对象的采用指针，不修改历史候选。"}
            </DialogDescription>
          </DialogHeader>
          {replacing ? (
            <Textarea
              ...
              placeholder="填写替换原因（必填）"
            />
          ) : null}
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setAdoptId(null)}>
              取消
            </Button>
            <Button
              type="button"
              disabled={adopt.isPending || (replacing && !reason.trim())}
              onClick={submitAdopt}
            >
              确认采用
            </Button>
      ...
      <Dialog
        open={rejectId !== null && canAdoptVersion}
        onOpenChange={(next) => {
          if (!next) {
            setRejectId(null)
            setReason("")
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>否决候选版本</DialogTitle>
            <DialogDescription>否决不会删除版本，原因将进入审计记录。</DialogDescription>
          </DialogHeader>
          <Textarea
            value={reason}
            onChange={(event) => setReason(event.target.value)}
            placeholder="填写否决原因（必填）"
          />
          <DialogFooter>
