...
function PublicationRow({
  ...
  work: AigcWork
  publication: AigcWorkPublication
    ...
    if (action.kind === "cancel") {
      ...
      cancel.mutate(
        {
          ...
          expectedPublicationVersion: publication.version,
          reason: reason.trim(),
        ...
        },
        {
          onSuccess: () => {
            setAction(null)
            setReason("")
            notify.success("Publication 已取消")
          }
        }
      )
    } else {
      retry.mutate(
        ...
        },
        {
          onSuccess: () => {
            setAction(null)
            notify.success("已创建新的关联重试 Publication，旧失败记录保留")
          }
        }
      )
    }
    ...
    <div className="flex flex-col gap-3 rounded-lg border p-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-xs">
            ...
            {publication.retryOfPublicationId
              ? ` · retry-of #${publication.retryOfPublicationId}`
              : ""}{" "}
            · 重试 {publication.retryCount} 次
          </p>
        </div>
        <Badge variant={STATUS_VARIANT[publication.status]}>
      ...
      {publication.status === "FAILED" ? (
        <Alert variant="destructive">
          <AlertTitle>{publication.failureCode || "渠道发布失败"}</AlertTitle>
          <AlertDescription>
            {publication.failureMessage || "渠道未返回可展示的失败说明"}
            {publication.responsePayload ? (
              <pre className="mt-2 max-h-32 overflow-auto text-xs">
                {JSON.stringify(publication.responsePayload, null, 2)}
              </pre>
            ) : null}
          </AlertDescription>
        </Alert>
      ...
      {publication.cancelReason ? (
        <p className="text-muted-foreground text-xs">取消原因：{publication.cancelReason}</p>
      ) : null}
      <div className="flex gap-2">
        {canPublish && active ? (
          <Button
            ...
            onClick={() => {
              ...
              setReason("")
              setAction({ kind: "cancel", publication })
            }}
          >
            <CircleStop />
            取消
        ...
        {canPublish && publication.status === "FAILED" ? (
          <Button
            type="button"
            size="xs"
            variant="outline"
            onClick={() => {
              if (!canPublish) return
              setAction({ kind: "retry", publication })
            }}
          >
            <RotateCcw />
            重试为新记录
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {action?.kind === "cancel" ? "取消 Publication" : "重试 Publication"}
            </DialogTitle>
            <DialogDescription>
              {action?.kind === "cancel"
                ? "正在发布的渠道任务将请求取消，原因会保留在历史。"
                : "重试将创建新的 Publication，并通过 retry-of 保留失败链路。"}
            </DialogDescription>
          </DialogHeader>
          {action?.kind === "cancel" ? (
            <Textarea
          ...
          <DialogFooter>
            ...
            <Button
              type="button"
              disabled={
                (action?.kind === "cancel" && !reason.trim()) || cancel.isPending || retry.isPending
              }
              onClick={execute}
            >
              确认
...
function WorkCard({
  ...
  const publishEnabled =
    canPublish && project?.status === "DELIVERING" && work.status !== "ARCHIVED"
  const archiveEnabled =
    canArchive && project?.status === "DELIVERING" && work.status !== "ARCHIVED"
      ...
      <GlassCardBody className="flex flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Project #{work.projectId} · DeliverableSet #{work.deliverableSetObjectId} · Manifest #
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            render={
              <Link
                href={`/studio/projects/${work.projectId}?focus=${work.deliverableSetObjectId}`}
              />
            }
            variant="outline"
          >
          ...
          {canPublish ? (
            <Button
              ...
              onClick={() => {
                if (publishEnabled) setPublishOpen(true)
              }}
            >
              <Plus />
              新建发布
          ...
          {canArchive ? (
            <Button
              ...
              onClick={() => {
                if (archiveEnabled) setArchiveOpen(true)
              }}
            >
              <Archive />
              归档 Work
        ...
        <div className="flex flex-col gap-3">
          <p className="font-medium text-sm">Publication 历史</p>
          {isLoading ? (
        ...
        <Dialog open={publishOpen && publishEnabled} onOpenChange={setPublishOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>创建 Publication</DialogTitle>
              <DialogDescription>
                仅创建正式发布请求；渠道结果由后端执行器写入，普通用户不可伪造成功。
              </DialogDescription>
            </DialogHeader>
            <Select
              value={channelSpecVersionId ? String(channelSpecVersionId) : ""}
              onValueChange={(value) => setChannelSpecVersionId(value ? Number(value) : null)}
            >
              <SelectTrigger className="w-full">
                <SelectValue>选择项目绑定渠道</SelectValue>
              </SelectTrigger>
              <SelectContent>
                <SelectGroup>
            ...
            <DialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  if (!project || !publishEnabled || !channelSpecVersionId) return
                  publish.mutate(
                    {
                      workId: work.id,
                    ...
                    },
                    {
                      onSuccess: () => {
                        ...
                        notify.success("Publication 已创建")
                      }
                    }
                  )
                }}
        ...
        <Dialog open={archiveOpen && archiveEnabled} onOpenChange={setArchiveOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>归档 Work</DialogTitle>
              <DialogDescription>
                Work 和全部 Publication 历史会保留，归档后不可继续发布。
              </DialogDescription>
            </DialogHeader>
            <Textarea
              value={archiveReason}
              onChange={(event) => setArchiveReason(event.target.value)}
              placeholder="填写归档原因（必填）"
            />
            <DialogFooter>
              ...
              <Button
                ...
                onClick={() => {
                  if (!project || !archiveEnabled || !archiveReason.trim()) return
                  archive.mutate(
                    {
                      workId: work.id,
                      expectedProjectVersion: project.version,
    ...
    <div className="relative h-full overflow-y-auto">
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-6 p-6">
        <header>
          ...
          <p className="text-muted-foreground text-sm">
            固定 approved、non-stale Manifest 的 Work 及不可变 Publication 历史。
          </p>
        </header>
        {isLoading ? (
          ...
          <div className="grid items-start gap-4 lg:grid-cols-2">
            {works.map((work) => (
              <WorkCard key={work.id} work={work} canPublish={canPublish} canArchive={canArchive} />
            ))}
          </div>
        )}
      </div>
