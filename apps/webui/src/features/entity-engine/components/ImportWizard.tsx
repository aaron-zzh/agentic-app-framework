  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-lg rounded-lg border bg-background p-6 shadow-xl">
        <h2 className="mb-4 text-lg font-semibold">导入 {entity.label}</h2>

        {step === "upload" && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">上传 CSV 文件</p>
        ...
        {step === "mapping" && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">字段映射（{headers.length} 列）</p>
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button type="button" className="rounded border px-3 py-1.5 text-sm" onClick={() => setStep("upload")}>上一步</button>
              <button type="button" className="rounded bg-primary px-3 py-1.5 text-sm text-primary-foreground" onClick={handleImport}>开始导入</button>
            </div>
          </div>
        )}
        ...
        {step === "result" && result && (
          <div className="space-y-3 text-center">
            <p className="text-lg font-medium">{result.errors === -1 ? "导入失败" : "导入完成"}</p>
            {result.errors !== -1 && (
              <p className="text-sm text-muted-foreground">成功 {result.success} 条，失败 {result.errors} 条</p>
            )}
            <button type="button" className="rounded bg-primary px-4 py-2 text-sm text-primary-foreground" onClick={onClose}>关闭</button>
          </div>
        )}
      </div>
    </div>
