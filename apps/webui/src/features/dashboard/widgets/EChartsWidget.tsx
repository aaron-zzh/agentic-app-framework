...
import { TrendChart, FunnelChart, RetentionChart, PieChart } from "@/features/stats/charts"
import { useStatsTrend, useStatsFunnel, useStatsRetention } from "@/lib/queries/use-stats"
...
export function EChartsWidget({ widgetId: _widgetId, title, config, refreshInterval }: EChartsWidgetProps) {
  return (
...
function ChartRenderer({ config, refreshInterval }: { config: EChartsWidgetConfig; refreshInterval?: number }) {
  switch (config.statsType) {
    ...
    case "pie":
      return <TrendRenderer config={{ ...config, chartType: "bar" }} refreshInterval={refreshInterval} />
    default:
      return <div className="flex h-full w-full items-center justify-center text-muted-foreground text-sm">未知图表类型</div>
  }
}

function TrendRenderer({ config, refreshInterval }: { config: EChartsWidgetConfig; refreshInterval?: number }) {
...
function EmptyState() {
  return <div className="flex h-full w-full items-center justify-center text-muted-foreground text-sm">暂无数据</div>
}
