...
const StartNode = memo(function StartNode(props: NodeProps) {
  return (
    <div
      className={cn(
        "flex h-10 w-10 items-center justify-center rounded-full border-2 border-green-500 bg-green-50",
    ...
    >
      <Handle
        ...
        className="!bg-green-500 !h-2 !w-2"
      />
      <span className="font-bold text-green-700 text-xs">开始</span>
    </div>
  )
})
...
const EndNode = memo(function EndNode(props: NodeProps) {
  return (
    <div
      className={cn(
        "flex h-10 w-10 items-center justify-center rounded-full border-2 border-red-500 bg-red-50",
        props.selected && "ring-2 ring-primary"
      )}
    >
      <Handle id="in" type="target" position={Position.Top} className="!bg-red-500 !h-2 !w-2" />
      <span className="font-bold text-red-700 text-xs">结束</span>
    </div>
  )
})
...
const GatewayNode = memo(function GatewayNode(props: NodeProps) {
  return (
    <div
      className={cn(
        "flex h-12 w-12 rotate-45 items-center justify-center border-2 border-amber-500 bg-amber-50",
        props.selected && "ring-2 ring-primary"
      )}
    >
      <Handle
        ...
        className="!bg-amber-500 !h-2 !w-2 !-rotate-45"
      />
      <Handle
        id="out-yes"
        type="source"
      ...
      <Handle
        ...
        className="!bg-amber-500 !h-2 !w-2 !-rotate-45"
      />
      <span className="-rotate-45 font-bold text-amber-700 text-xs">×</span>
    </div>
  )
})
...
function UserTaskInspector({ data, onChange }: InspectorProps) {
  return (
    <div className="space-y-3">
      ...
      <div>
        <label htmlFor="assigneeStrategy" className="font-medium text-sm">
          审批人策略
        </label>
        <select
          id="assigneeStrategy"
      ...
      <div>
        <label htmlFor="assignee" className="font-medium text-sm">
          审批人
        </label>
        <input
          id="assignee"
          ...
          placeholder={
            // biome-ignore lint/suspicious/noTemplateCurlyInString: Flowable expression
            data.assigneeStrategy === "EXPRESSION" ? "${expression}" : "输入用户名或角色"
          }
        />
      </div>
      <div>
        <label htmlFor="timeoutStrategy" className="font-medium text-sm">
          超时策略
        </label>
        <select
          id="timeoutStrategy"
      ...
      <div>
        <label htmlFor="emptyAssigneeStrategy" className="font-medium text-sm">
          空审批人处理
        </label>
        <select
...
function ServiceTaskInspector({ data, onChange }: InspectorProps) {
  return (
    <div className="space-y-3">
      <div>
        <label htmlFor="serviceClass" className="font-medium text-sm">
          服务类
        </label>
        <input
          id="serviceClass"
...
function CountersignInspector({ data, onChange }: InspectorProps) {
  return (
    <div className="space-y-3">
      <div>
        <label htmlFor="countersignMode" className="font-medium text-sm">
          会签模式
        </label>
        <select
          id="countersignMode"
      ...
      {data.countersignMode === "RATIO" && (
        <div>
          <label htmlFor="passRatio" className="font-medium text-sm">
      ...
      <div>
        <label htmlFor="assignees" className="font-medium text-sm">
          审批人列表
        </label>
        <input
          id="assignees"
        ...
        />
        <p className="mt-1 text-muted-foreground text-xs">多个审批人用逗号分隔</p>
      </div>
    </div>
  )
}
