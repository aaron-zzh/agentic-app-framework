...
import type { ApprovalFlowNode, ApprovalFlowBranch, FormFieldDef } from "./types"
...
export function VerticalDesigner({ value, onChange, formFields = [] }: VerticalDesignerProps) {
  ...
  /** 在指定节点后插入新节点 */
  const insertAfter = useCallback((parentId: string, newNode: ApprovalFlowNode) => {
  ...
  /** 更新节点配置 */
  const updateNode = useCallback((nodeId: string, config: Record<string, unknown>) => {
  ...
  /** 更新分支条件 */
  const updateBranch = useCallback((nodeId: string, branchId: string, patch: Partial<ApprovalFlowBranch>) => {
    function update(node: ApprovalFlowNode): ApprovalFlowNode {
      if (node.id === nodeId) {
        return {
          ...node,
      ...
      <div className="flex justify-center py-2">
        <div className="relative flex h-8 items-center">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => insertAfter(parentId, {
                id: genId(), type: "approver", name: "审批人", config: {}, next: undefined
              })}>
                👤 审批人
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => insertAfter(parentId, {
                id: genId(), type: "cc", name: "抄送人", config: { timing: "ON_APPROVE" }, next: undefined
              })}>
                📋 抄送人
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => insertAfter(parentId, {
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span>{icon}</span>
              <span className={cn("text-sm font-medium", colors.text)}>{node.name}</span>
            </div>
            {canDelete && (
              <Button
                variant="ghost"
      ...
      <div className="flex justify-center">
        <div className="rounded-lg border border-l-4 border-l-purple-500 bg-purple-50 p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-sm font-medium text-purple-700">🔀 {node.name}</span>
            <Button
              variant="ghost"
              size="sm"
              className="h-5 w-5 p-0 text-muted-foreground hover:text-destructive"
          ...
          <div className="flex gap-4">
            {node.branches.map((branch) => (
              <div key={branch.id} className="min-w-[180px] rounded border bg-white p-2">
                <button
                  type="button"
                  className="mb-2 w-full text-left text-xs font-medium text-purple-600 hover:underline"
                ...
                {!branch.child && (
                  <div className="flex justify-center py-1">
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="outline" size="sm" className="h-6 w-6 rounded-full p-0 text-xs">+</Button>
                      </DropdownMenuTrigger>
                      <DropdownMenuContent>
                        ...
                        <DropdownMenuItem onClick={() => {
                          updateBranch(node.id, branch.id, {
                            child: { id: genId(), type: "cc", name: "抄送人", config: { timing: "ON_APPROVE" } }
                          })
                        }}>
                          📋 抄送人
                        </DropdownMenuItem>
  ...
  function ConfigContent() {
    ...
    if (selectedNode.type === "condition" && selectedNode.config._editBranch) {
      const branch = selectedNode.branches?.find(
        (b) => b.id === selectedNode.config._editBranch
      )
      if (!branch) return null
      return (
        <div className="space-y-4">
          <div>
            <label htmlFor="branch-name" className="text-sm font-medium">分支名称</label>
            <input
              id="branch-name"
    ...
    return <p className="text-sm text-muted-foreground">该节点无可配置属性</p>
  }
