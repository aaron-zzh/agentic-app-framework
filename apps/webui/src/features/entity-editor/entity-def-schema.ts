...
export const entityDefJsonSchema = {
  ...
  type: "object",
  required: ["kind", "resource", "label", "fields", "listView"],
  properties: {
    kind: { type: "string", const: "code", description: "代码实体类型" },
    resource: { type: "string", description: "受信任代码资源标识" },
    ...
    FieldDef: {
      ...
      properties: {
        ...
        displayModes: {
          type: "array",
          description: "显示的表单模式",
          items: { type: "string", enum: ["create", "edit"] }
        },
        visibleRoles: {
          type: "array",
          description: "拥有任一角色时才显示字段（仅控制前端展示）",
          items: { type: "string" }
        },
        ...
        relationTo: { type: "string", description: "关联目标的受信任资源标识" },
        writeKey: { type: "string", description: "引用对象写入 DTO 的字段名" },
        idValueType: { type: "string", enum: ["number", "string"] },
        allowedEntitySlugs: { type: "array", items: { type: "string" } },
        excludeEntitySlugs: { type: "array", items: { type: "string" } },
