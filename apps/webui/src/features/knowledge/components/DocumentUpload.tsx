...
export function DocumentUpload({ knowledgeBaseId }: DocumentUploadProps) {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setUploads((prev) =>
        prev.map((u) => (u.id === id ? { ...u, status: "failed", error: (err as Error).message } : u))
      )
    }
  }, [knowledgeBaseId, qc])
          ...
          {uploads.map((item) => (
            <div key={item.id} className="flex items-center gap-3 rounded-md border p-3">
              ...
              <Badge variant={item.status === "completed" ? "default" : item.status === "failed" ? "destructive" : "secondary"}>
                {item.status === "uploading" ? `${item.progress}%` : item.status === "completed" ? "完成" : "失败"}
              </Badge>
            </div>
          ))}
        </div>
...
function StatusIcon({ status }: { status: UploadItem["status"] }) {
  switch (status) {
    case "uploading": return <Loader2 className="text-primary size-5 animate-spin" />
    case "completed": return <CheckCircle className="size-5 text-green-500" />
    case "failed": return <XCircle className="text-destructive size-5" />
  }
}
