...
const { isDark, toggleTheme, followSystem, setFollowSystem } = useManualTheme()
...
<template>
  <view class="min-h-screen py-3">
    ...
    <!-- 主题切换 -->
    <demo-block title="主题切换" transparent>
      <view class="rounded-2 p-4 wot-bg-filled-oppo">
        <view class="mb-3 text-4 font-bold wot-text-text-main">
          暗黑模式切换
        </view>
        ...
        <wd-button type="primary" block @click="toggleTheme()">
          切换主题
        </wd-button>
        <view class="mt-3">
          <wd-button
            :type="followSystem ? 'primary' : 'default'"
            block
            @click="setFollowSystem(!followSystem)"
          >
            {{ followSystem ? '✅ 跟随系统' : '跟随系统（已关闭）' }}
          </wd-button>
        </view>

        <!-- 主题色预览 -->
        <view class="mt-4 text-3 font-bold wot-text-text-main">
          主题色预览
        </view>
        <view class="mt-2 flex gap-2">
          <view class="h-8 flex-1 rounded-2" style="background: var(--wot-primary-6);" />
          <view class="h-8 flex-1 rounded-2" style="background: var(--wot-filled-content);" />
          <view class="h-8 flex-1 rounded-2" style="background: var(--wot-border-main);" />
          <view class="h-8 flex-1 rounded-2 border border-gray-200" style="background: var(--wot-filled-oppo);" />
        </view>
        <view class="mt-1 flex gap-2 text-center text-2 wot-text-text-auxiliary">
          <text class="flex-1">primary-6</text>
          <text class="flex-1">filled</text>
          <text class="flex-1">border</text>
          <text class="flex-1">oppo</text>
        </view>
      </view>
    </demo-block>
  </view>
