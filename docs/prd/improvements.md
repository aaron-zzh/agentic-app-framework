...
- 2026-08-26 | **执行期输入分类不信任客户端**：当前信任客户端 kind，`MODIFY/SUPPLEMENT` 仅支持澄清补参。2026-08-29 人类确认方案 **C：完整动态重规划+不可变 BoardRevision**（非分阶段路线的中间态，直接采用完整语义）——MODIFY 语义边界明确包含目标/约束修改，**也包含执行计划/DAG 修改**（新增/删除步骤、改变依赖、角色或工具），不局限于澄清字段纠正；取消统一走既有确定性 `/stop` 端点，通用输入通道不再信任客户端 CANCEL；分类为 MODIFY 后生成新计划候选，人工确认后创建不可变 `BoardRevision`（记录 `baseVersion`/`createdBy`/`reason`/`inputId`），同一时刻只有一个 active version，激活新版本需与 lease fencing、父任务状态、outbox 事件原子提交；v1 运行中节点默认中断未完成部分，保留已发生事实，不自动复用结果（除非确定性兼容检查通过）；execution/event/result 全部绑定 boardVersion。涉及 TaskBoard 模型、表结构、端口、事务、事件、调度、恢复与前端的架构级改动，待立项设计与技术任务拆分 | 已采纳（待立项）
