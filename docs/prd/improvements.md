...
- 2026-08-26 | **Workflow 第二正文通路**：`WorkflowAgUiController` 自建正文 SSE 与事件投影，违反"AG-UI 是唯一正文通道"。2026-08-27 复核修正了判定：该端点前端唯一调用点是 `flow-editor-view.tsx:76` 的 `handleDebug()`，`debug` 恒为 true，实际承担的是编排画布试跑；而 `requireRunnableFlow` 里的非 debug 生产分支（要求 flow `PUBLISHED`）**零调用方**，那才是真正违反唯一正文通道的部分。已按人类确认拆两步：**第一步已完成**——`/api/workflow/run` 收窄为仅调试（`debug` 非真直接拒绝、仅创建者），删除非 debug 生产分支（禁双路径），前端同步去掉 `startWorkflow` 的 `debug` 形参恒传 `true`；`runtime.md` 新增两通道契约对照表（入口/调用方/准入/正文语义/事件与状态/人工节点六维），"唯一正文通道"与"正文投影"实现态改为已实现。2026-08-29 补充：三个设计点中"`LlmNode` 是否收进统一链"已解决——按人类确认方案，`AgentNode` 与 `LlmNode` 合并为统一节点（删除 `LlmNode.java`），改为构造 `AssistantCommand` 走 `AssistantCommandPort`（不再是裸 `AgentExecutionPort`），以工作流触发用户身份 + 单轮任务式（`TASK`+`FIXED` route，`memoryMode=DISABLED`）执行；节点未配置 `roleKey` 时回退该用户默认 Assistant 的 `defaultRoleKey`，`skillKey` 可留空。`AiFlowBpmnCompiler` 节点映射表 `"llm"`/`"agent"` 均指向同一 delegate `agentNode`。同时补齐：`WorkflowTool.startWorkflow`/`AgentCallableWorkflowPort.start()` 语义改为同步等待完成（轮询 `isProcessRunning`+`getInstance`，5 分钟超时，`terminated` 状态视为失败），返回结束节点 `output` 变量文本；新增 `subworkflow` 节点类型，编译为 Flowable `<callActivity>`（`flowable:inheritVariables="true"`，含直接自引用防护）。新增测试：`AgentNodeTest`（4）、`AgentCallableWorkflowServiceTest`（5）、`WorkflowToolTest`（2）、`AiFlowBpmnCompilerTest` 新增 3 个子工作流测试，`pnpm nx test service` aaf-framework 371/371 全绿。**其余两个设计点与第二步（真正的 `processMode=PREDEFINED_WORKFLOW` 执行分支）仍待立项**：工作流节点是否映射为 SubTask、调试临时部署在统一运行时如何表达；`PREDEFINED_WORKFLOW` 判定分支在生产链路 `workflowKey` 仍恒为 `null`（`AssistantExecutionService.java` 调用点），未接通任何触发入口，属独立场景不在本次范围 | 已完成（第一步 + LlmNode 合并/同步等待/子工作流）· 第二步（PREDEFINED_WORKFLOW 执行分支）待立项
...
- 2026-08-26 | **三层恢复闭环**：通用步骤级、会话重建、目标级恢复及恢复后授权复核未闭合。2026-08-29 人类确认方案 **C：先闭合"安全边界重启"最小子集**——不追求 ReAct 内部任意步骤续跑，L2 在 receipt/节点完成边界重启；为 DelegatedTask/TaskBoard 增加类型化 checkpoint（版本/hash/eventOffset）；统一 `RecoveryPreflight`（恢复前重新校验主体/grant/凭证/策略/预算，任一不确定 fail-closed）；恢复默认新建 execution 不迁移旧 fence 的 AgentState；已完成写动作只凭成功 receipt+下游幂等/资源版本跳过，无法证明则转人工；Team/Goal 版本漂移一律不自动迁移、转人工重规划。接口与状态归属按分层方向（L2/L3/L4 各自建设+共享恢复门禁）预留，不做统一 Recovery Aggregate（避免形成第二状态真理源）。2026-08-30 完成核心实现：新增 `TaskCheckpoint`（record：version/contentHash/eventOffset/annotations，序列化仍落原 `ai_delegated_task.task_payload` JSONB 列，不新增列），`DelegatedTask.checkpoint` 字段类型化；新增 `RecoveryPreflight` 端口 + `DefaultRecoveryPreflight` 默认实现（任务归属/终态/deadline/预算四项静态可判定检查，grant/凭证细粒度校验仍在下次工具调用时由 `DefaultToolGateway` 校验，范围边界已在类注释说明），接入 `DelegatedTaskCoordinator.recoverAndDispatch`。**未完成**：L3 会话完整重建（原子加载 TaskBoard+任务状态+事件 offset+节点画像引用的协调器）与 L4 目标级恢复（Goal/Team 独立生命周期）仍未实现，当前只有 L2 步骤级安全边界重启落地 | 进行中（L2 步骤级已完成；L3/L4 未实现）
...
- 2026-08-26 | **产物四工具与 `ARTIFACT_*` 事件**：仅 `content.draft.upsert` 已实现，`artifact.reserve/checkpoint/commit/fail` 及对应事件缺失，长产物无法断点续跑。2026-08-29 人类曾确认方案 **A：完整实现四工具与四事件**；2026-08-30 复核后**撤销该方案，判定为不做**。撤销原因：① 排查发现当前系统里没有任何触发路径会选中"Reservation 优先"时序——`ArtifactPolicy.saveTool` 唯一被真实赋值的地方只有 `content.draft.upsert`（`SkillService.java:867` 强制文案生成类 Skill 绑定该工具），四个新工具做出来会是零调用方的死代码，与此前 `TaskRecoveryPort.checkpoint()`/`ContextLoadTool` 挂起续跑方案同类问题；② 真正的"长内容生成"现状是流式一次性吐出+整体落库，不是"多次物理调用分段产出"，四工具解决的"生成中断续跑"并非当前架构的实际执行方式；③ 排查"分段长文档"类场景（论文/博客）后发现真正缺失的是**协调者拆分后多个子任务产物如何聚合成一份结构完整的最终文档**（`coordination.md` 现有 `ORDERED_CONCAT` 仅做文本拼接，不处理文字+图片混排），这与"生成过程 checkpoint 防丢失"是两个不同问题，后者需求更明确，前者当前无实证需求。若未来出现"多次物理调用产出同一产物、且中途可能中断"的真实场景（当前唯一候选是预定义工作流分段生成，但 `processMode=PREDEFINED_WORKFLOW` 分支本身未接通），再重新评估设计 | 已撤销（不做）
- 2026-08-26 | **执行期输入分类不信任客户端**：当前信任客户端 kind，`MODIFY/SUPPLEMENT` 仅支持澄清补参。2026-08-29 人类确认方案 **C：动态重规划，终止冲突节点+原地更新 Board**（2026-08-30 复核收窄，去除版本化设计）——MODIFY 语义边界明确包含目标/约束修改，**也包含执行计划/DAG 修改**（新增/删除步骤、改变依赖、角色或工具），不局限于澄清字段纠正；取消统一走既有确定性 `/stop` 端点，通用输入通道不再信任客户端 CANCEL；服务端新增非自主分类器重新判定输入意图，不信任客户端标注的 `kind`。分类为 MODIFY 后：先终止冲突的运行中/未执行子任务（复用既有 `TaskBoard.interruptRunning`，终止后的节点状态天然拒绝旧执行流程的滞后写回，不需要额外并发保护机制）；再按新目标重新走一次协调规划（复用既有 Coordinator 重规划机制生成新 `subTasks`）；直接原地更新 `ai_task_board` 这一行（`(tenant_id, task_id)` 唯一约束不变，不新增表或版本字段）。**不引入 `BoardRevision`/版本号/历史快照**——审计留痕交给既有事件流（`ai_task_event` 追加一条"Board 已修改"事实），不在 Board 实体上重复记录版本信息；已完成节点结果不自动复用到新计划。涉及 `ExecutionInput`/`InputBuffer`/`TaskBoard` 调度分支/`JpaTaskTransitionAdapter.consumeInputs` 核心逻辑改造。2026-08-30 完成实现：`ExecutionInput` 删除 `CANCEL` 枚举值（取消统一走既有确定性 `/stop` 端点），新增 `text` 字段承载原始自然语言输入；新增 `InputClassifier` 端口 + `DefaultInputClassifier`（非自主 L0 模型调用，仿照 `DefaultRoleSelector` 安全默认模式，异常/非法输出均 fail-closed 退回 `UNRELATED` 不放大为 `MODIFY`）；`TaskIngress` 接入分类器，写入前用判定结果覆盖客户端 `kind`；`JpaTaskTransitionAdapter.consumeInputs` 重写 MODIFY 分支——无待处理澄清且 Board 含 `coordinator` 节点时，用新目标构造全新 `TaskBoard` 整体替换（`single` 类型 Board 无 `coordinator`，不支持重规划，按未生效处理，fail-closed 不猜测替代语义）。跨端契约同步：webui `DelegatedTaskInputRequest` 去掉 `CANCEL`、`content` 改名 `text` | 已完成
- 2026-08-26 | **短期会话上下文压缩**：`memory.md` 契约要求短期记忆为"压缩摘要"，当前实现为原文按预算裁剪注入，压缩这步未做。2026-08-29 人类确认方案 **C：分层增量压缩**——新建独立会话语义端口（如 `SessionContextCompressionPort`），不直接复用执行级 `ContextCompressionPort`（作用对象与生命周期不同），但复用其底层 LLM 调用/超时/计量/严格 JSON 校验/防提示注入基础设施；Redis 同时维护"已冻结结构化摘要 + 最近原文尾部"，未超阈值只读原文，超字符/轮次阈值后对被挤出窗口的旧轮次异步摘要（不阻塞主执行）；最近 4-8 条/2-4 轮原文始终保留原文精度；摘要严格区分"已确认决定/已验证事实/未决问题"，无来源不得升级为事实，统一标记为低信任上下文并携带 `sourceMessageIds`；LLM 失败/超时/校验失败时保留旧摘要+最新尾部，不覆盖不阻断。2026-08-30 完成实现：新增 `SessionContextCompressionPort`/`DefaultSessionContextCompressor`（摘要字段收敛为 `confirmedDecisions`/`verifiedFacts`/`openQuestions` 三类，硬性拒绝阈值设为告知模型目标长度的 2 倍容差，避免模型小幅超出即整次摘要作废）；`ShortTermMemoryService` 新增摘要存取（独立 Redis key，不新建表）与 `trimToRecent` 裁剪方法；`RedisSessionMemoryAdapter` 原文超阈值时异步生成摘要并冻结，成功后裁剪已被摘要吸收的旧原文（避免摘要与原文重复发送），Redis 摘要和原文都失效时从会话历史数据库（`ConversationHistoryPort`/`JpaConversationHistoryAdapter`）兜底读取并回填；`SessionMemoryPort` 新增 `DEFAULT_MAX_TURNS` 公共常量统一召回读取数与存储层裁剪保留数，避免两处硬编码不一致 | 已完成
...
- 2026-09-01 | **内置工具生产可用性（`ai_tool_catalog` 种子数据缺失 + `ActionEffect` 猜测逻辑不准确）**：AAF-107 #10703 实现 `submit_executor_plan` 工具时发现两处耦合缺口：① 全仓 migration 无任何 `INSERT INTO ai_tool_catalog`，`support.handoff`/`context.load` 等既有内置工具在生产环境同样缺目录种子数据，需管理员后台手工注册才可用；② `ToolPolicy.ActionEffect` 不是从 `ai_tool_catalog.read_only` 读出来的，而是 `JpaAssistantDefinitionAdapter.effect(String tool)` 按工具名字符串关键词猜测（含 "handoff"→HUMAN_HANDOFF，"delete"/"publish"→IRREVERSIBLE_WRITE，"create"/"update"/"write"→REVERSIBLE_WRITE，"generate"→GENERATED_CONTENT，其余默认 READ），`submit_executor_plan` 这类名字不含关键词的工具会被误分类（本例功能不受影响，因为误分类结果 `READ` 恰好也在 `READ_ONLY` 模式下被允许，但语义不准确）；③ 两层配置必须一致，`DefaultToolGateway.invoke` 会核对 `rule.readOnly() != catalogReadOnly` 等条件不一致直接拒绝执行，若只补种子数据不核对 `effect()` 猜测结果，可能产生"配置了但运行时被拒"的排查陷阱。人类已确认暂不处理（选项 C），建议单独立项统一解决种子数据 + 分类逻辑，不要分两次改同一问题 | 待评估
- 2026-09-01 | **AAF 工具权限机制对比官方 AgentScope Permission System 的缺口**：AAF-107 #10704 启动前核实官方 `permission-system.html` 后的对比结论——AAF 在持久化/多副本/资源粒度授权（`AuthorizationGrant` 带过期时间与资源级 scope）、事务性审批状态机（`ai_hitl_approval` 落库不丢，官方 `ConfirmResult` 靠内存态 metadata 往返）、幂等 receipt（`InvocationReceiptPort`）三方面比官方更完整，但缺两项官方有的运行时安全加固：① **Built-in Checks**（`ToolBase#checkPermissions`，工具自己按真实调用参数做动态检查，不可被 mode/rules 覆盖，如检测目标是否为生产资源）——AAF 当前只按工具名静态属性（`readOnly`/`reversible`）判定，不深入检查具体参数内容；② **危险资源硬编码黑名单**（`ToolDangerousPathConstants`，`.env`/`.ssh`/`.git` 等命中即 ASK，连 `BYPASS` 模式都不能绕过）——AAF 没有等价的不可覆盖清单，全靠人工配置 `ai_tool_catalog`/`ToolPolicy` 静态属性，无运行时兜底。已登记为 [AAF-109](../task/backlog.md) | 已采纳（未实施）
- 2026-09-02 | **`io.agentscope.core.util.JsonSchemaUtils` classpath shadow 的正式退出路径登记**（AAF-108 #10803）：该类是 AAF 在自己源码树里同包同名覆盖 AgentScope core jar 内实现的兼容 shim（`apps/service/aaf-framework/src/main/java/io/agentscope/core/util/JsonSchemaUtils.java`），因上游 2.0.2 仍用 Jackson 2 + victools 4 编译，AAF 因 Spring AI on Jackson 3 必须钉 victools 5，两边 API 不兼容，删除即 `NoSuchMethodError`，已在 ADR-005「补充定案：`JsonSchemaUtils` shadow 保留」与类注释中确认**保留并登记为「禁兼容层」硬规则的显式例外**（AAF-103 落地时已完成类注释登记，本条补齐改进意见池的正式记录，避免例外只留在代码注释里无处追踪）。**退出路径**（任一达成即可移除 shadow）：① 向 AgentScope 上游提交 Jackson 3 / victools 5 兼容 PR 并等待 GA；② 若上游长期未响应，改为维护独立坐标的最小 AgentScope fork，整体替换官方依赖坐标并删除本 shadow 类。退出时机判定：每次升级 AgentScope 版本时检查上游 `JsonSchemaUtils` 是否已切换到 victools 5——一旦满足，本类必须立即删除，不得因"暂时还能跑"而保留双份实现 | 已采纳（保留为显式例外，退出路径待触发）
- 2026-09-03 | **webui 缺少 AG-UI `ActivitySnapshot`/`ActivityDelta` 自定义渲染逻辑**（AAF-104 #10403，已排期为独立前端任务）：后端已按 AG-UI 协议标准实现子任务活动卡片投影（`InternalNodeEventConverter`/`AafAguiStreamContext.subTaskActivityChanged`，`activityType="SUBTASK"`，随子任务 `EXECUTION_STARTED/COMPLETED/FAILED/CANCELED` 发 Snapshot/Delta）。**已修正的判断**：`useAgUiRuntime` 默认渲染逻辑确实只认 `activityType="a2ui-surface"`（配合官方 A2UI 生成式 UI 协议），其它 `activityType` 默认不产生 UI，但这不代表前端无法消费——`@ag-ui/client` 的 `AgentSubscriber` 接口本身提供标准 `onActivitySnapshotEvent`/`onActivityDeltaEvent` 回调（与现有 `AgUiChatProvider` 已在用的 `onCustomEvent`/`onRunStartedEvent` 同级别、同一订阅机制 `agent.subscribe({...})`），webui 可以直接接入，成本可控，不依赖 assistant-ui 未来版本才能支持。 | 已排期（前端任务见 AAF-104 dev-log）
...
- 2026-08-30 | **`aaf-api` 测试代码存在既有编译错误（阻塞 `test-compile`）**：`AgentCallableWorkflowServiceTest.java:46,65,83` 对 `findOne` 的引用不明确——`JpaSpecificationExecutor` 同时存在 `findOne(PredicateSpecification<T>)` 和 `findOne(Specification<T>)` 两个重载均匹配，可能是 Spring Data JPA 版本升级引入的新重载导致。`git status` 确认该文件与近期改动无关，是此前未被发现的技术债（此前从未跑过 `test-compile` 才未暴露）。修复方向：显式转型消除歧义，或改用不重载的具体方法。当前阻塞 `aaf-api` 模块整体测试编译，需尽快处理 | 待评估
