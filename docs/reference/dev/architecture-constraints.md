...
Flyway 通过 `spring.flyway.locations`（`classpath:db/migration,classpath:db/seed[,classpath:db/testdata]`）扫描 `db/migration`（schema）与 `db/seed`（种子数据）两个目录，版本号（`v{N}`）是**跨目录全局单序列**——Flyway 按数值大小统一排序执行，不区分子目录、不区分 schema/seed，因此不能在两个目录下各自独立编号。

**历史号段（v1-v99，已固定，不再变动）**：v1-v10 为基础模块 schema，v11-v14 为早期 seed，v15-v16 为 workspace 相关迁移。混杂 schema/seed 是历史遗留，不追溯整理。

**模块号段（v100 起）**：从 v100 开始，每个新增业务板块独占一个百位号段，号段内该模块的 schema 变更与 seed 数据混合累加使用，不再按 migration/seed 分类：
...
- 新增业务板块首次需要迁移脚本时，向本表登记一个新的空闲百位号段（v200-299、v300-399……依次递增），不与已登记号段重叠
- 号段内文件命名不变：`v{N}__{描述}.sql`，`N` 必须落在该模块登记的号段范围内，按时间顺序递增，不跳号
- 单号段（100 个版本号）预期覆盖单个模块的全生命周期迭代；确有特殊情况用尽时，顺延登记下一个空闲号段，不影响其他模块
- 文件仍分别放在 `db/migration/`（schema）与 `db/seed/`（种子数据）目录下，号段只约束数字范围，不改变目录归属
- 号段登记只能由协调者更新本表；开发者发现号段冲突或需要新号段时，在 `dev-log.md` 中提出，不得自行分配
