...
五度约束适用于**分类型目录**（按主题/类型组织），**不适用于实例集合目录**：

- `_template/`（模板集合）— 模板数量由流程环节决定，不受 5 限制
- `snippets/`（代码片段集合）— 片段按需增长
- `roles/`（角色定义集合）— 角色数量由团队结构决定
- `adr/`（决策记录集合）— ADR 按时间累积

判断标准：目录内的文件是**同类实例的平铺**（如 N 个模板、N 个角色），而非**不同主题的分类**。
