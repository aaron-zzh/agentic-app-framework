...
| 能力 | 说明                                                                                                                                                                                              |
|------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 认知层（Cognition） | Memory + Knowledge + Value（价值观） + Retrieval（融合检索） + Learning 横切，基于 PgVector/Neo4j/Redis 自实现，借鉴 Mem0/Graphiti/Cognee/LightRAG，不集成 Python 框架。详见 [认知层设计](../../framework/intelligent/bak/cognition/cognition.md) |
...
| 运行时 | 版本 | 核心技术 | 说明 |
|-------|------|---------|------|
| **实体运行时** | v0.2+ | ALTER TABLE 动态加列 + JPA MetaModel + 自动生成 API | DSL 定义实体 → 动态建表 → 自动 CRUD；用户自定义字段为未实现提案，设计稿已移除 |
