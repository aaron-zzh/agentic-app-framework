...
| 能力 | 核心体验 | 关键边界 | 阶段 |
|---|---|---|---|
| 灵感广场 | 浏览作品、公开 Prompt 摘要、使用的创作方式与脱敏制作过程；可从项目类型或公开快照 Remix | 不展示系统 Prompt、私有素材、客户资料和未授权模型参数 | MVP 后 |
...
项目图谱采用 **React Flow（`@xyflow/react`）作为视口与交互渲染引擎**，自研 `ProjectGraph` 领域模型、双视图投影、关系语义、折叠聚合和布局策略；结构视图使用普通 React 组件。tldraw 只作为可选的自由灵感板/标注工具，不承担项目图谱；不自研底层画布引擎。

| 方案 | 适用能力 | 本方案结论 |
|---|---|---|
| React Flow | 类型化业务节点、自定义卡片、语义边、选择、拖拽、缩放、小地图 | **项目图谱主引擎** |
| tldraw | 自由绘图、Moodboard、圈选标注、参考图自由编排、协作白板 | 按需打开的次级工具 |
...
自由画布作为项目内可选对象存在，而不是默认工作台：

```text
项目图谱中的“灵感板 / 参考板”节点
  → 按需打开 tldraw
    → 自由排列图片、手绘和批注
      → 将选区导出或关联为 ProjectAsset
```

React Flow 与 tldraw 不在同一主工作面同时加载；tldraw 按需懒加载，其 Shape/Store 也不能成为 `ProjectGraph` 真理源。
