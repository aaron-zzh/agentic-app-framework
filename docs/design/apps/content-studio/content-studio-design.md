...
当前 WebUI 已在 FlowEditor、项目图谱和文档关系图中使用 React Flow，并具备自定义节点/边、拖放、连接、选择、缩放、小地图和撤销/重做基础；知识库实体图谱因核心知识探索需要，已独立采用 `@aaf/knowledge-visualization` + Three.js。Studio 与实体画板已使用 tldraw，但主要面向自由绘图、导出和协作，自定义领域 Shape 尚未形成成熟基线。因此项目图谱仍复用 React Flow 技术基础，不把知识库的 WebGL 引擎扩散为通用业务画布。
