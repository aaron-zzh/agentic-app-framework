...
```text
1. AgentScope 是 Agent 层的执行骨架，AAF 五层是薄门面（每层 ~50-100 行）
2. 多模态能力：Agent 内部走 AgentScope 原生；非 Agent 场景走 ai/image/ 封装
3. 认知循环分层原则：每层有且只有一个认知循环，粒度从项目级到请求级逐层细化；上层循环通过调度触发下层循环，下层结果通过回调返回上层，禁止跨层直接触发
4. 决策权跨层流动：低置信度决策上报上层处理，高置信度决策本层直接执行，决策权随置信度在层间动态流动，不固定归属某层
5. 能力护栏：Assistant 根据任务类型动态限定 Agent 操作范围（工具白名单+权限边界），限定范围换取信任空间，减少人工审查成本
6. 瓶颈迁移意识：执行近乎免费，规划与审查是新瓶颈——Agent 核心价值是帮用户规划和审查
```
...
权限系统的"主体"不是 User，而是 **Operator**——Human 和 AI 的多态抽象：
