...
| 特性 | 一句话说明 | 详细设计 |
|---|---|---|
| 开发时能力 | 核心引擎开发（规范→DSL→代码生成→热部署）+ 业务系统开发（四层无代码运行时覆盖 ~80% 场景） | [dev-capability.md](core/dev-capability.md) |
| 运行时能力 | 工作流执行、智能体编排、知识记忆集成、智能降级、沙箱隔离 | [runtime-capability.md](core/runtime-capability.md) |
| 自进化机制 | 引擎自进化（强制人工审核）+ 业务系统自进化（置信度分级），共用 `aaf-auto-dev` | [evolution.md](core/evolution.md) |
| 知识能力一体 | 工具与知识库强绑定，运行时强制一致性校验，「工具执行 → 知识生长」正向闭环 | [metadata-manager.md](core/metadata-manager.md) · [nexus-knowledge.md](engine/nexus-knowledge.md) |
| 预算感知 | 四维预估（Token/时间/工具调用/费用）+ 三档监控（70%/90%/100%）+ 用户决策 | [budget-control.md](engine/budget-control.md) |
| 人类计算支撑 | 用户贡献群体智慧，积分引擎量化，结算引擎链上透明 | [human-computation.md](core/human-computation.md) |
| 复杂性封装 | 默认隐藏，四层按需展开；查看无需权限，修改按角色分级 | [complexity-encapsulation.md](core/complexity-encapsulation.md) |
| 对话式交互 | 意图表达与结果呈现，与元引擎通过 DSL 和事件解耦 | [conversational-interaction.md](../apps/webui/tmp/conversational-interaction.md) |

## 技术映射

详见 [代码结构设计](code-structure.md#技术映射)。
...
元引擎定期扫描 `docs/` 与 `aaf-modules/` 的语义一致性，发现偏离时触发告警并生成修正任务，不依赖人工检查。详见 [自进化机制 · 引擎自进化触发条件](core/evolution.md)。
