...
1. ✅ #1 PageDef 类型定义 — developer-webui
...
2. ✅ #2 PageEngine 渲染器 — developer-webui (依赖: #1)
...
3. ✅ #3 PageDef 存储与加载 — developer-service + developer-webui (依赖: #2)
...
4. ✅ #4 NavbarSection + FooterSection — developer-webui (依赖: #2)
   - Navbar：logo + 导航链接 + CTA + sticky + 移动端汉堡菜单
   - Footer：链接分组 + 社交媒体 + 版权
   - 响应式适配
   - verify: 各断点布局正确

5. ✅ #5 HeroSection — developer-webui (依赖: #2)
   - 主标题 + 副标题 + CTA 按钮组
   - backgroundType：gradient / image / particles / plain
   - 响应式：桌面居中 / 移动端全宽
   - verify: 不同 backgroundType 正确渲染

6. ✅ #6 FeaturesSection — developer-webui (依赖: #2)
   - 功能卡片网格（图标 + 标题 + 描述）
   - columns 配置（2/3/4 列）
   - 响应式：桌面多列 / 手机单列
   - verify: 6 个卡片在各断点正确排列

7. ✅ #7 PricingSection — developer-webui (依赖: #2)
   - 定价卡片（方案名 + 价格 + 功能列表 + CTA）
   - highlighted 推荐标记
   - 功能对比 ✓/✗
   - verify: 三列定价卡片正确渲染

8. ✅ #8 ShowcaseSection + StatsSection + FAQSection — developer-webui (依赖: #2)
   - Showcase：Tab 切换产品截图/动画
   - Stats：数据统计（数字动画）
   - FAQ：折叠面板（Accordion）
   - verify: 各组件交互正确

9. ✅ #9 CTASection + TestimonialsSection + LogosSection — developer-webui (依赖: #2)
   - CTA：行动号召横幅
...
13. ✅ #13 页面编辑器 UI — developer-webui (依赖: #3)
