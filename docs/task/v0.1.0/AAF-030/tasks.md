...
> 负责人：architect + developer-webui | 创建：05-13
...
1. [ ] #3001 关联字段组件（RelationshipPicker） — developer-webui
...
2. [ ] #3002 级联选择组件（Cascader） — developer-webui (依赖: #3001)
   - 多级关联（省→市→区模式）
   - `dependsOn` 配置级联依赖
   - 上级变更自动清空下级 + 重新加载选项
   - verify: 选择省份后城市列表正确过滤

3. [ ] #3003 文件上传组件（Upload） — developer-webui
   - 拖拽上传 + 点击选择
   - 前端校验（类型/大小/数量）
   - 上传进度条
   - 图片预览（lightbox）+ 缩略图
...
4. [ ] #3004 富文本编辑器组件（RichText） — developer-webui
   - 基于 Tiptap 实现
   - 工具栏：标题/粗体/斜体/链接/图片/代码块/引用/表格
   - 图片粘贴/拖拽上传
   - verify: 富文本编辑保存后内容正确渲染

5. [ ] #3005 签名字段组件（Signature） — developer-webui
...
6. [ ] #3006 子表明细行组件（Subtable） — developer-webui (依赖: #3001)
   - 内联表格：增删改行 + 行拖拽排序
   - 汇总行（sum/avg/count）
...
7. [ ] #3007 多币种/单位字段组件 — developer-webui
   - Money 字段：值 + 币种选择器
   - Quantity 字段：值 + 单位选择器
   - 显示换算值（showConverted）
...
8. [ ] #3008 FieldContext 统一表达式上下文 — developer-webui
...
9. [ ] #3009 条件可见性引擎 — developer-webui (依赖: #3008)
...
12. [ ] #3012 公式字段引擎 — developer-webui (依赖: #3008)
    - `type: 'formula'` 字段实时计算
...
13. [ ] #3013 跨字段校验规则 — developer-webui (依赖: #3008)
...
14. [ ] #3014 离开确认（Unsaved Changes Guard） — developer-webui
