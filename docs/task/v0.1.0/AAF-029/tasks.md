...
1. [ ] #02901 useEntityList Hook — developer-web
...
2. [ ] #02902 useEntityRecord / useEntityMutation / useEntityDelete — developer-web (依赖: #02901)
...
3. [ ] #02903 后端通用 CRUD API — developer-api
...
4. [ ] #02904 nuqs URL 状态集成 — developer-web (依赖: #02901)
...
9. [ ] #02909 筛选构建器 — developer-web (依赖: #02904)
   - [+ 添加筛选] 弹出筛选面板
...
10. [ ] #02910 筛选收藏 — developer-web (依赖: #02909)
    - [保存为收藏] → 命名 → 存入 localStorage / 后端 user_preference
    - 收藏列表展示 + 一键应用 + 设为默认
    - 支持团队共享（后端存储）
    - verify: 保存/加载/删除收藏筛选正确

11. [ ] #02911 全局搜索（跨实体） — developer-web + developer-api (依赖: #02901)
...
12. [ ] #02912 列表导出 — developer-web + developer-api (依赖: #02901)
    - 工具栏 [导出] 按钮 → 选择格式（CSV/XLSX）+ 选择字段
    - 后端 `GET /api/{entity}/export?format=csv&fields=...`
    - 前端触发下载
    - verify: 导出 CSV 文件内容与列表数据一致

13. [ ] #02913 数据导入向导 — developer-web + developer-api (依赖: #02912)
...
14. [ ] #02914 嵌套导入（主从关联） — developer-web + developer-api (依赖: #02913)
...
15. [ ] #02915 Server Actions 前端触发 — developer-web (依赖: #02902)
...
16. [ ] #02916 透视视图 — developer-web + developer-api (依赖: #02901)
    - 视图切换器新增 [📊 透视] Tab
    - 维度面板：拖拽字段到行/列/值区域
...
17. [ ] #02917 批量操作异步化 — developer-web + developer-api (依赖: #02902)
    - 阈值判断：≤100 同步，>100 异步
