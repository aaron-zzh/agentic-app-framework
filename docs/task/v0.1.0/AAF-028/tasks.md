...
1. [ ] #1 EntityDef 类型定义 — developer-web
   - 定义 `EntityDef` / `FieldDef` / `ListViewConfig` / `FormViewConfig` / `KanbanViewConfig` 完整 TypeScript 接口
   - 定义 `EntityAccess` / `FieldAccess` 权限接口
   - 放置于 `packages/entity-engine/src/types/`（或 `apps/webui/src/lib/entity-engine/types/`，视 AAF-023 #18 落地结构而定）
   - verify: `pnpm nx run webui:typecheck` 通过

2. [ ] #2 Mixin 与继承机制 — developer-web (依赖: #1)
...
3. [ ] #3 实体注册表（Entity Registry） — developer-web (依赖: #2)
...
4. [ ] #4 组件注册表（Component Registry） — developer-web (依赖: #1)
...
5. [ ] #5 ViewEngine 核心渲染器 — developer-web (依赖: #3, #4)
...
7. [ ] #7 ListView 基础实现 — developer-web (依赖: #5)
...
8. [ ] #8 KanbanView 基础实现 — developer-web (依赖: #5)
   - 基于 @dnd-kit 实现看板视图
...
9. [ ] #9 FormView 基础实现 — developer-web (依赖: #5)
...
10. [ ] #10 工作区布局壳 — developer-web (依赖: #6)
...
11. [ ] #11 视图切换与 Toolbar — developer-web (依赖: #10, #7)
...
12. [ ] #12 分层错误边界 — developer-web (依赖: #5, #9)
...
13. [ ] #13 基础字段组件集 — developer-web (依赖: #4)
...
14. [ ] #14 加载状态与骨架屏 — developer-web (依赖: #7, #9)
    - 列表骨架屏：匹配列数/行数
    - 表单骨架屏：匹配字段数
    - 路由切换顶部进度条
