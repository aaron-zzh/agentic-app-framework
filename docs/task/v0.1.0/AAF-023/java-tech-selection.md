...
| **认知层（Cognition）** | Memory（记忆）+ Knowledge（知识库）+ Value（价值观）+ Retrieval（融合检索）四组件 + Learning 横切反哺通道，基于 PgVector+Neo4j+Redis 自实现，借鉴 Mem0/Graphiti/Cognee/LightRAG 精华，不集成 Python 框架。详见 [认知层设计](../../../design/framework/core/cognition/Readme.md) |
