...
- ⚠️ **补记（2026-09-02，在 AAF-107 #10705 编码时发现）**：AAF-110 提交（`f08a2a29`）当时只跑了 `pnpm nx compile service`（人类明确要求"最后一起编译"），未跑 `pnpm nx test service`，遗漏了 `HarnessAgentExecutionAdapterTest` 的 12 处测试回归——`#11004` 新增的 `GracefulShutdownManager.bindStateSaver(agent, ...)`/`registerRequest(agent)` 内部用 `ConcurrentHashMap` 以 `agent.getAgentId()` 为 key，测试用的 Mockito mock `agent` 未 stub `getAgentId()` 默认返回 `null`，`ConcurrentHashMap.put(null, ...)` 直接抛 `NullPointerException` 中断整条 `executeResolved` 执行链，导致 12 个测试用例全部走不到 `streamEvents(...)`。已在 `HarnessAgentExecutionAdapterTest.setUp()` 补充公共桩 `lenient().when(agent.getAgentId()).thenReturn("agent-test")` 修复（生产环境 `agentId` 由 `AgentBase` 构造时赋值永不为空，纯粹是 mock 契约缺失，不是生产代码缺陷）。用 `git stash` 隔离验证确认这批失败在 AAF-110 提交后即已存在，与 #10705 改动无关。**教训**：`compile` 通过不代表 `test` 通过——涉及第三方单例/静态状态（如 core `GracefulShutdownManager.getInstance()`）的改动，即使只跑 `compile` 作为阶段性检查点，也应该在任务收口前补跑一次 `test` 确认没有引入运行期回归，不能完全依赖"最后一起编译"的约定跳过。已在本条记录里补上这一步验证，`pnpm nx test service` 现在全绿。
